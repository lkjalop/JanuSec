...
def test_entra_connector_signins_and_audits(tmp_path):
    ...
    def _request_json(kind, payload):
        if kind == 'signins':
            return {'value': [{'id': 'signin-1', 'userPrincipalName': 'alice@example.com', 'ipAddress': '203.0.113.10', 'createdDateTime': '2026-01-01T00:00:00Z'}]}
        return {'value': [{'id': 'audit-1', 'activityDisplayName': 'Add member to role', 'activityDateTime': '2026-01-02T00:00:00Z', 'initiatedBy': {'user': {'userPrincipalName': 'bob@example.com'}}}]}
