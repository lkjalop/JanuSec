...
async def test_iam_privilege_edges():
    ...
    assert ('user:alice','policy:AdministratorAccess','configured_exposure') in edges
...
async def test_cloudtrail_high_risk_edges():
    ...
    assert ('ip:1.2.3.4','event:CreateAccessKey','candidate_match') in edges
    assert not any(edge[2] == 'observed_causal' for edge in edges)
    assert any(item.get('reason') == 'observed_causal_requires_evidence_ids' for item in g['excluded_edges'])
