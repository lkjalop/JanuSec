...
def _headers(tenant='default'):
    return {'x-api-key': API_KEY if tenant == 'default' else 'test-' + tenant, 'x-tenant-id': tenant}


@pytest.fixture(autouse=True)
def isolated_deliveries(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('API_KEYS_JSON', json.dumps([
        {'key': API_KEY, 'tenant_id': 'default', 'scopes': ['*']},
        {'key': 'test-fixturetenant', 'tenant_id': 'fixturetenant', 'scopes': ['*']}]))
...
def test_connector_fixture_poll_path_is_guarded_and_populates_health(client):
    cfg = client.put(
        ...
        headers=_headers('fixturetenant'),
        json={
            'config': {
                'fixture_events': [
                    {
    ...
    status = client.get('/api/v1/connectors/fixturetenant/email/mimecast/status', headers=_headers('fixturetenant'))
    assert status.status_code == 200
    payload = status.json()['status']
