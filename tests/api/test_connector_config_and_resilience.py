...
def tenant_credentials(monkeypatch, tmp_path):
    from src.integrations import tenant_store
    monkeypatch.setattr(tenant_store, 'STORE_DIR', str(tmp_path / 'tenant_store'))
    monkeypatch.setenv('POLLING_STATE_DIR', str(tmp_path / 'polling'))
    tenants = ['cfg-tenant', 'dedupe-tenant', 'circuit-tenant']
    monkeypatch.setenv('API_KEYS_JSON', json.dumps([
        {'key': 'test-' + tenant, 'tenant_id': tenant, 'scopes': ['*']} for tenant in tenants]))
    monkeypatch.setenv('JANUSEC_CONNECTOR_RECEIPTS_DB', str(tmp_path / 'deliveries.sqlite'))


def _headers(tenant):
    return {'x-api-key': 'test-' + tenant, 'x-tenant-id': tenant}
...
def test_azure_connector_config_roundtrip(client):
    ...
    get_resp = client.get(f'/api/v1/connectors/{tenant}/azure/entra_signin/config', headers=_headers(tenant))
...
def test_connector_dedupes_repeated_polls(monkeypatch, client):
    ...
    first = client.post(f'/api/v1/connectors/{tenant}/azure/entra_signin/poll', headers=_headers(tenant), json={'since_ts': 0})
    second = client.post(f'/api/v1/connectors/{tenant}/azure/entra_signin/poll', headers=_headers(tenant), json={'since_ts': 0})
...
def test_connector_circuit_state_surfaces_on_repeated_failure(monkeypatch, client):
    ...
    for _ in range(3):
        resp = client.post(f'/api/v1/connectors/{tenant}/azure/entra_signin/poll', headers=_headers(tenant), json={'since_ts': 0})
        assert resp.status_code == 502
    dry = client.post(f'/api/v1/connectors/{tenant}/azure/entra_signin/poll', headers=_headers(tenant), json={'dry_run': True})
