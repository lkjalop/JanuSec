...
def test_live_lane_assessment_round_trip_includes_email_and_connector_provenance(monkeypatch, client):
    ...
    case_view = client.get(f"/api/v1/assessments/{assessment['assessment_id']}/case-view", headers=_headers())
    assert case_view.status_code == 200, case_view.text
    assert case_view.json()['case']['tenant_id'] == 'default'
    assert case_view.json()['report_context']['historical_receipt']['receipt_hash']
