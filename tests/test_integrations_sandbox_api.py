...
def test_set_config_and_list_tasks(tmp_path):
    ...
    body = r.json()
    assert body.get('saved') is True or 'path' in body

    # list tasks (should return structure even if empty)
    r2 = client.get('/api/v1/admin/sandbox/tasks', headers=_hdrs)
    assert r2.status_code == 200
    data = r2.json()
    assert 'count' in data and 'tasks' in data

    # schedule a refresh (won't error even if task is unknown)
    r3 = client.post('/api/v1/admin/sandbox/refresh', params={'task_id':'sim-123'}, headers=_hdrs)
