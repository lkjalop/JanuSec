...
pytestmark = [
    pytest.mark.timeout(600),  # module fixture evaluates three full corpora
...
@pytest.mark.parametrize("scenario", ["vesper", "meridian"])
def test_attribution_truth_has_no_missed_cases_or_role_failures(results, scenario):
    assert results[scenario]["fn"] == [], f"{scenario}: required case not detected"
    assert results[scenario]["separation_failures"] == [], f"{scenario}: independent cases were over-stitched"
    assert results[scenario]["role_failures"] == [], f"{scenario}: actor/victim/target role attribution failed"


@pytest.mark.parametrize("scenario", ["vesper", "meridian", "santos"])
def test_role_aware_partition_truth_is_release_authority(results, scenario):
    truth = results[scenario]["authoritative_truth"]
    if scenario != "santos":
        assert truth["must_detect"] == 1.0
    assert truth["must_suppress"] == 1.0
    if scenario == "meridian":
        assert truth["must_separate"] == 1.0
    # Santos has no labeled role expectations; do not manufacture a perfect score.
    assert truth["role_attribution"] == (None if scenario == "santos" else 1.0)
...
# Attribution grading now holds all three corpora at zero confirmed breach
# inflation from one-sensor telemetry with no attributable subject or causal path.
# Current locked release invariant: zero confirmed unattributed network cases.
_FP_NETWORK_BREACH_BASELINE = {"vesper": 0, "santos": 0, "meridian": 0}
