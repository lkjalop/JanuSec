...
def test_ip_resolution_requires_strong_dominance_and_preserves_ambiguous_nat():
    strong = [{"src_ip": "10.0.0.8", "user_canonical": "martin.chen"} for _ in range(8)]
    target = {"src_ip": "10.0.0.8", "target_account": "svc_sql"}
    resolve_entities(strong + [target])
    assert target["user_canonical"] == "martin.chen"
    assert target["_entity_owner_method"] == "ip_cooccurrence_strong"

    shared = ([{"src_ip": "10.0.0.9", "user_canonical": "alice"} for _ in range(5)] +
              [{"src_ip": "10.0.0.9", "user_canonical": "bob"} for _ in range(5)])
    unknown = {"src_ip": "10.0.0.9"}
    resolve_entities(shared + [unknown])
    assert not unknown.get("user_canonical")
