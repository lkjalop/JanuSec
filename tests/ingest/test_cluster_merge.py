...
def test_t1_attribution_safe_cases_cover_all_phases_without_weak_union(fixture):
    """T1 — preserve phase cases until evidence stronger than shared IP/CIDR,
    device, hostname, operation, or fingerprint supports a causal merge.

    Behavioral links may recommend a campaign; they must not rewrite case truth.
    """
    _, clusters = fixture
    campaigns = _by_kind(clusters, "campaign")
    assert 3 <= len(campaigns) <= 10
    for c in campaigns:
        assert c["verdict"] in {"VALIDATED_BREACH", "SUSPECTED_BREACH", "NO_VALIDATED_BREACH"}
...
def test_t6_cidr24_is_topology_not_case_identity(fixture):
    """T6 — a shared /24 is retained in evidence but cannot union cases."""
    rows, clusters = fixture
    from src.core.ingest.cluster_merge import build_scope_qualified_pivots

    # These two addresses are also joined through a named principal elsewhere
    # in the fixture, so campaign membership cannot isolate the CIDR variable.
    # Assert directly on the correlation inputs: topology prefixes are retained
    # on rows but never become identity pivots.
    pivots = build_scope_qualified_pivots(rows)
    assert not any(key.startswith(("cidr24:", "src_cidr24:", "dst_cidr24:")) for key in pivots)
    raw_ips = {value for row in rows for value in (row.get("src_ip"), row.get("dst_ip")) if value}
    assert {"45.133.193.42", "45.133.193.118"} <= raw_ips
...
def test_t7_snowflake_bau_does_not_enter_campaign(fixture):
    ...
    leaked = [
        i for campaign in _by_kind(clusters, "campaign") for i in campaign["row_refs"]
...
def test_t8_dns_beacon_collapses_by_signature(fixture):
    ...
    _, clusters = fixture
    beacon_phases = [
        phase for campaign in _by_kind(clusters, "campaign")
        for phase in campaign["phases"] if phase["phase_id"] == "c2_dns_beacon"
    ]
    assert len(beacon_phases) == 1
    beacon_phase = beacon_phases[0]
...
def test_t9_top_card_count_bounded(fixture):
    ...
    # Preserve attribution-safe cases; presentation pages/groups them without
    # rewriting evidence into fewer campaigns.
    assert not any(c.get("_campaign_overflow") for c in surfaced)
...
def test_t10_provenance_phase_row_refs_resolve(fixture):
    ...
    rows, clusters = fixture
    rows_by_idx = {r["row_index"]: r for r in rows}
    for campaign in _by_kind(clusters, "campaign"):
...
def test_unclassified_bucket_exists(fixture):
    """BAU remains in raw evidence and never needs weak union to make a card."""
    rows, clusters = fixture
    unc = _by_kind(clusters, "unclassified")
    assert not any("SHOW TABLES" in str(c) for c in _by_kind(clusters, "campaign"))
    assert any("SHOW TABLES" in str(row.get("event")) for row in rows)
...
def test_detector_anomaly_label_is_not_a_union_pivot():
    from src.core.ingest.cluster_merge import build_scope_qualified_pivots

    rows = [
        {"row_index": 1, "user_canonical": "alice", "src_ip": "198.51.100.10", "_janusec_anomaly": "user_ip_drift"},
        {"row_index": 2, "user_canonical": "bob", "src_ip": "203.0.113.20", "_janusec_anomaly": "user_ip_drift"},
    ]
    pivots = build_scope_qualified_pivots(rows)

    assert "seed:user_ip_drift" not in pivots
    assert pivots["canon_user:alice"] == [1]
    assert pivots["canon_user:bob"] == [2]


def test_ordinary_named_user_collection_plus_rdp_is_not_validated():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    cluster = {
        "verdict": "VALIDATED_BREACH",
        "final_verdict": "VALIDATED_BREACH",
        "shared_users": ["lisa.petrov"],
        "shared_ips": ["10.11.78.201"],
        "phases": [
            {"phase_id": "bastion_rdp_lateral"},
            {"phase_id": "cloud_object_collection"},
        ],
    }
    _regrade_verdicts([cluster])
    assert cluster["verdict"] == "NO_VALIDATED_BREACH"


def test_cloud_collection_plus_external_mail_requires_two_sources_to_validate():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    phases = [{"phase_id": "cloud_object_collection"}, {"phase_id": "email_external_exfil"}]
    one_source = {"verdict": "VALIDATED_BREACH", "phases": phases, "sources": ["audit.json"]}
    two_sources = {
        "verdict": "VALIDATED_BREACH",
        "phases": phases,
        "sources": ["cloud-audit.json", "mail-audit.json"],
    }
    _regrade_verdicts([one_source, two_sources])
    assert one_source["verdict"] == "NO_VALIDATED_BREACH"
    assert two_sources["verdict"] == "VALIDATED_BREACH"


def test_rdp_or_wmi_only_is_context_not_a_suspected_breach():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    for phase_id in ("bastion_rdp_lateral", "wmi_dcom_lateral"):
        cluster = {
            "verdict": "VALIDATED_BREACH",
            "final_verdict": "VALIDATED_BREACH",
        ...
        _regrade_verdicts([cluster])
        assert cluster["verdict"] == "NO_VALIDATED_BREACH"
        assert cluster["correlation_requirement"] == "multi_source_causal_support_missing"


def test_named_principal_cloud_collection_only_is_context():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    cluster = {
        "verdict": "VALIDATED_BREACH",
        "shared_users": ["ordinary.user"],
        "shared_ips": ["10.0.0.5"],
        "phases": [{"phase_id": "cloud_object_collection"}],
    }
    _regrade_verdicts([cluster])
    assert cluster["verdict"] == "NO_VALIDATED_BREACH"


def test_single_source_external_mail_plus_admin_activity_is_context():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    cluster = {
        "verdict": "VALIDATED_BREACH",
        "shared_users": ["ordinary.user"],
        "source_artifacts": ["mail-audit.json"],
        "phases": [
            {"phase_id": "email_external_exfil"},
            {"phase_id": "bastion_rdp_lateral"},
        ],
    }
    _regrade_verdicts([cluster])
...
def test_multi_source_ambiguous_activity_requires_typed_causal_support():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    cluster = {
        "verdict": "SUSPECTED_BREACH",
        "shared_users": ["ordinary.user"],
        "sources": ["endpoint.json", "mail.json"],
        "phases": [
            {"phase_id": "email_external_exfil"},
            {"phase_id": "bastion_rdp_lateral"},
        ],
    ...
    _regrade_verdicts([cluster])
    assert cluster["verdict"] == "NO_VALIDATED_BREACH"
    assert cluster["correlation_requirement"] == "typed_causal_support_missing"


def test_ambiguous_activity_can_remain_suspected_with_evidence_backed_causal_edge():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    cluster = {
        "verdict": "SUSPECTED_BREACH",
        "sources": ["endpoint.json", "network.json"],
        "phases": [
            {"phase_id": "bastion_rdp_lateral"},
            {"phase_id": "c2_dns_beacon"},
        ],
        "typed_edges": [{
            "source": "process:p1", "target": "network:connection-1",
            "edge_type": "observed_causal", "evidence_ids": ["ev-process", "ev-network"],
        }],
    }
    _regrade_verdicts([cluster])
...
def test_validated_multi_source_ambiguous_activity_without_causality_becomes_context():
    from src.core.ingest.cluster_merge import _regrade_verdicts
...
def test_external_email_requires_attachment_direction_and_success():
    from src.core.ingest.cluster_merge import _det_email_external_exfil

    base = {
        "external_recipient_domain": "outside.example",
        "attachment_count": 1,
        "action_direction": "mailbox_to_external_recipient",
        "action_outcome": "success",
    }
    assert _det_email_external_exfil(base, "") is True
    assert _det_email_external_exfil({**base, "attachment_count": 0}, "ccp-exfil sinobiz") is False
    assert _det_email_external_exfil({**base, "action_outcome": "denied"}, "") is False
    assert _det_email_external_exfil({**base, "action_direction": "external_to_mailbox"}, "") is False


def test_as_rep_requested_account_is_target_not_implicit_actor():
    from src.core.ingest.cluster_merge import _derive_case_roles

    roles = _derive_case_roles(
        [{"EventID": 4768, "PreAuthType": 0, "account_name": "svc_sql", "user": "svc_sql"}]
    )
    assert {item["entity"] for item in roles if item["role"] == "target"} == {"svc_sql"}
    assert not [item for item in roles if item["role"] == "actor"]


def test_anonymous_external_cloud_collection_can_remain_validated():
    from src.core.ingest.cluster_merge import _regrade_verdicts
...
def test_unattributed_single_sensor_signature_is_context_not_breach():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    # Randomized values make the invariant independent of any acceptance
    # fixture name, address, or campaign label.
    cluster = {
        "verdict": "VALIDATED_BREACH",
        "final_verdict": "VALIDATED_BREACH",
        "shared_users": [], "shared_hosts": [],
        "shared_ips": ["10.77.33.19"],
        "sources": ["random-network-sensor.ndjson"],
        "phases": [{"phase_id": "ransomware_staging"}],
    }
    _regrade_verdicts([cluster])
    assert cluster["verdict"] == "NO_VALIDATED_BREACH"
    assert cluster["correlation_requirement"] == "attribution_or_independent_corroboration_missing"


def test_named_host_or_causal_lineage_prevents_overbroad_suppression():
    from src.core.ingest.cluster_merge import _regrade_verdicts

    named_host = {
        "verdict": "VALIDATED_BREACH", "shared_hosts": ["random-host-942"],
        "sources": ["random-endpoint-sensor.ndjson"],
        "phases": [{"phase_id": "ransomware_staging"}],
    }
    causal = {
        "verdict": "VALIDATED_BREACH", "shared_users": [], "shared_hosts": [],
        "sources": ["random-network-sensor.ndjson"],
        "phases": [{"phase_id": "ransomware_staging"}],
        "typed_edges": [{
            "source": "process:11", "target": "socket:22",
            "edge_type": "observed_causal", "evidence_ids": ["ev-11", "ev-22"],
        }],
    }
    _regrade_verdicts([named_host, causal])
    assert named_host["verdict"] == "VALIDATED_BREACH"
    assert causal["verdict"] == "VALIDATED_BREACH"
