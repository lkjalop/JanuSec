...
_VESPER = os.path.join("tests", "fixtures", "telemetry_corpora", "Vesper")
...
_SHADOW_SHAPE_CASES = [
    ...
    ("kerberos_ticket_forgery", {"windows_event_id": "4769", "ticket_options": "0x60a10000",
...
def test_golden_ticket_is_not_conflated_with_kerberoasting():
    row = {"windows_event_id": "4769", "ticket_options": "0x60a10000",
           "account_name": "martin", "event_id": "hash"}
    tags = detect_row_phase_tags(row)
    assert "kerberos_ticket_forgery" in tags
    assert "kerberoasting" not in tags
