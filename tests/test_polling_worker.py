...
def isolated_secret_store(monkeypatch, tmp_path):
    # This legacy module imports both package aliases below. Isolate both.
    import sys
    directory = tmp_path / 'isolated_secrets'
    directory.mkdir()
    monkeypatch.setenv('TENANT_STORE_DIR', str(directory))
    for name in ('src.integrations.tenant_store', 'integrations.tenant_store'):
        module = sys.modules.get(name)
        if module is not None:
            monkeypatch.setattr(module, 'STORE_DIR', str(directory))
...
def test_token_refresh_path(monkeypatch, tmp_path):
    import src.integrations.tenant_store as store_module
    store_dir = tmp_path / 'tenant_store'
    store_dir.mkdir()
    monkeypatch.setenv('TENANT_STORE_DIR', str(store_dir))
    monkeypatch.setattr(store_module, 'STORE_DIR', str(store_dir))
