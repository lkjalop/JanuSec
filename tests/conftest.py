        ...
        class _WrappedCounter:
            ...
            def inc(self, v: float = 1.0, labels=None):
                try:
                    ...
                    pass
            def labels(self, *args, **labels):
                # Real prometheus_client accepts positional label VALUES mapped onto
                # the metric's labelnames (e.g. .labels("crowdstrike", "signature")).
                # The shim previously accepted only keywords, so any positional call
                # raised TypeError. Map positional args onto labelnames to match.
                if args:
                    for name, val in zip(self._labelnames, args):
