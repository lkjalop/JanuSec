...
def test_cumulative_exfil_attaches_to_actor_cluster():
    ...
    phase = next(p for p in cluster["phases"] if p["phase_id"] == "cloud_object_exfiltration")
    assert phase["direction"] == "outbound"
    assert phase["row_refs"] == list(range(30))
