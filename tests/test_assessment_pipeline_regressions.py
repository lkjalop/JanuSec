...
def test_kill_chain_legacy_lead_prefers_validated_case_over_longer_suspected_case():
    assessment = {
        "case_partitions": [
            {"case_id": "suspected", "verdict": "SUSPECTED_BREACH"},
            {"case_id": "validated", "verdict": "VALIDATED_BREACH"},
        ],
        "case_chronologies": [
            {"case_id": "suspected", "milestones": [{"phase": "unknown"}] * 20, "unsequenced": []},
            {"case_id": "validated", "milestones": [{"phase": "initial_access"}], "unsequenced": []},
        ],
    }
    clusters = [{"cluster_id": "validated", "final_verdict": "VALIDATED_BREACH", "row_refs": []}]

    assessment_worker._seed_proposed_actions_and_kill_chain(assessment, clusters, [])

    assert assessment["kill_chain_case_id"] == "validated"
...
def test_partition_owned_roles_prevent_service_account_actor_inversion():
    from src.core.ingest.assessment_worker import (
        _cluster_from_case_partition,
        _generate_deterministic_executive_summary,
    )

    partition = {
        "case_id": "case-martin",
        "title": "Martin multi-stage intrusion",
        "verdict": "VALIDATED_BREACH",
        "row_refs": [1, 2],
        "phase_ids": ["kerberoasting", "oauth_device_code"],
        "roles": [
            {"role": "actor", "entity": "martin.chen"},
    ...
    legacy_targets = [
        {
            ...
            "shared_users": ["svc_backup"],
            "factor_tags": ["iam:as_rep_roasting"],
            "phases": [{"phase_id": "kerberoasting"}],
        }
    ]
    projected = _cluster_from_case_partition(partition, legacy_targets)
    assert projected["shared_users"] == ["martin.chen"]
    assert {item["entity"] for item in projected["entity_roles"] if item["role"] == "target"} == {
        "svc_backup", "svc_sql"
    }
    result = _generate_deterministic_executive_summary(
        {}, [projected], [{"row_index": 1, "timestamp": "2026-04-12T00:00:00Z", "user": "martin.chen"}],
    )
    assert "Principal actors: martin.chen" in result["executive_summary"]
    assert "Principal actors: svc_backup" not in result["executive_summary"]
