...
def test_upload_creates_sessions_and_builds_graph():
    ...
    node_types = set(n.get('kind') for n in j['graph']['nodes'])
    assert 'evidence' in node_types
    assert any(n for n in j['graph']['nodes'] if n.get('kind') != 'evidence')
...
def test_graph_nodes_have_evidence_counts():
    ...
    ent_nodes = [n for n in nodes if not n.get('node_id','').startswith('session:')]
