...
Quick curl snippets (assumes server on http://localhost:8080 and default dev key `${API_KEY}`)
...
```bash
curl -sS http://localhost:8080/api/v1/csv/deep_analyze/auto_backfill/<ASSESSMENT_ID>/status -H "x-api-key: ${API_KEY}"
```
