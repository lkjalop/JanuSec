...
// Helper to wait for content to change
async function waitForNonEmptyText(locator: ReturnType<typeof test['step']> | any) {
  await expect(locator).not.toHaveText(/No LLM triage summary yet\.|LLM triage failed|LLM triage error/i, { timeout: 10000 });
}
...
test('Missing logs panel shows default and click handler', async ({ page }) => {
  ...
  await btn.click();
  // Notification is a div with class .notification; assert it appears
