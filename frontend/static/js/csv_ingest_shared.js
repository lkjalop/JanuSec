...
(function(){
  ...
  function detectMapping(rows){
    ...
    const mapping = {};
    const sample = rows[0];
    // Sanitize headers: trim, collapse whitespace, remove control chars
    const headers = Object.keys(sample || {}).map(function(h){
      try{
  ...
  async function ingestRows(options){
    // Client-side size caps and validation
    const rows = Array.isArray(options.rows) ? options.rows : [];
    const maxRows = Number(localStorage.getItem('CSV_MAX_ROWS')) || 200000;
    const maxCols = Number(localStorage.getItem('CSV_MAX_COLS')) || 200;
    const cleaned = [];
    for(let i=0;i<rows.length && i<maxRows;i++){
      const r = rows[i];
      if(!Array.isArray(r) && typeof r !== 'object') continue;
      const obj = Array.isArray(r) ? r : Object.values(r);
      if(obj.length > maxCols) continue;
      cleaned.push(obj.map(function(cell){
        if(cell == null) return '';
        let s = String(cell);
        // Strip control chars and excessively long cells
        s = s.replace(/[\u0000-\u001F]+/g,'');
