...
(function(){
  ...
  function getPipelineStatusForRow(rec, idx){
    ...
    if(PIPELINE_PENDING && PIPELINE_PENDING.has(idx)) return 'pending';
    if(PIPELINE_TARGETS && PIPELINE_TARGETS.has(idx)) return 'queued';
    return 'none';
  }
  ...
  function computePipelinePriority(rec, idx){
    ...
    try{
      ...
      }else if(PIPELINE_PENDING && PIPELINE_PENDING.has(idx)){
  ...
  async function pushIncidentForRow(rowIndex){
    try{
      ...
      if(row.llm_summary){
        ...
        description = row._tier1_summary;
      }else{
