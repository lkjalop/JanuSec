...
Revision ID: 0002_repair_decisions_table
...
revision = '0002_repair_decisions_table'
down_revision = '0001_create_reports_table'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not _has_table(bind, 'events'):
        ...
        # Older JanusSec deployments used timestamp/event_data/verdict fields.
        # Repair the additive ingest columns before creating their indexes; do
        # not rename or drop legacy columns because they may contain live data.
        event_columns = {
            'source': sa.Column('source', sa.Text(), nullable=True),
            'event_type': sa.Column('event_type', sa.Text(), nullable=True),
            'severity': sa.Column('severity', sa.Text(), nullable=True),
            'ts_ingested': sa.Column('ts_ingested', ts_type, nullable=True, server_default=sa.func.now()),
            'ts_original': sa.Column('ts_original', ts_type, nullable=True),
            'raw_payload': sa.Column('raw_payload', json_type, nullable=True),
            'tenant_id': sa.Column('tenant_id', sa.Text(), nullable=True),
        }
        for name, column in event_columns.items():
            if not _has_column(bind, 'events', name):
