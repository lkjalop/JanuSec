import os, time
os.environ['INCIDENTS_DEBUG']='1'
os.environ['PLATFORM_LITE_INIT']='1'
os.environ['TEST_HELPERS_ENABLED']='1'
from fastapi.testclient import TestClient
from src.api.server import app
from src.core.graph.hopgraph_lite import get_graph

client=TestClient(app)
# setup graph
g=get_graph(); g.edges_ts.clear(); g.observe({'edge_type':'auth','user':'dave','host':'ha'}); g.observe({'edge_type':'net','host':'ha','peer':'hb'})
seed={'user':'dave','host':'ha'}
persisted=[]
class FakeRepo:
    def __init__(self): self._mem=[]
    async def persist(self, rec): self._mem.append(rec); persisted.append(rec)
    async def list_recent_decisions(self, limit=50, tenant_id=None): return list(self._mem[-limit:])
    def list_memory(self): return list(self._mem)
fake=FakeRepo()
import src.api.server as server_mod
server_mod.decisions_repo = fake
class FakeIncidentsRepo:
    async def upsert_incident(self, incident_id, payload, tenant_id=None):
        await fake.persist({'event_id': incident_id, **payload})
server_mod.incidents_repo = FakeIncidentsRepo()

r = client.post('/api/v1/graph/reconstruct?attach_incident=true', json=seed, headers={'x-api-key':'devkey123'})
print('reconstruct status', r.status_code, r.text)
import time as _t; _t.sleep(0.2)
print('persisted count', len(persisted))
if persisted: print('persisted[0]=', persisted[0])
# append to in-memory incident store
rec = persisted[0]
server_mod._INCIDENT_STORE.append({'id': rec.get('event_id') or rec.get('id') or f"inc-{int(time.time()*1000)}", 'metadata': rec.get('metadata') or {}, 'tenant_id': rec.get('tenant_id'), 'ts': time.time()})
server_mod.decisions_repo = None
server_mod.DECISIONS_REPO = None
r2 = client.get('/api/v1/incidents', headers={'x-api-key':'devkey123'})
print('incidents list status', r2.status_code)
print('incidents body:', r2.json())

# Direct call to list_incidents to inspect returned value
import asyncio
res = asyncio.run(server_mod.list_incidents(None, limit=50, tenant_id=None))
print('direct list_incidents returned:', res)
