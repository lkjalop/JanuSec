...
## Recorded case history and connector test isolation

- The canonical case-view and GRC action-pack endpoints share tenant, assessment, case and `as_known_at` scope. Recorded projections live in `JANUSEC_CASE_HISTORY_DB` (default: `SESSION_PERSIST_DIR/case_history.sqlite`). Do not replace historical records with current derivations or round cutoffs to minutes. Dates before capture remain explicitly evidence-only.
- The case selector endpoint accepts `as_known_at` and returns recorded case metadata for historical requests. Keep the console URL, selector and exported `historical_receipt` aligned.
- `GET /api/v1/assessments/{id}/grc-action-pack?format=oscal&customer_profile_id={profile}` uses an explicitly provisioned tenant profile under `JANUSEC_CUSTOMER_PROFILE_DIR`. It exports candidate evidence without declaring control effectiveness or an audit opinion.
- Entra normalized delivery receipts use `JANUSEC_CONNECTOR_RECEIPTS_DB`; acknowledge a pending checkpoint only after durable delivery. Customer tenant and Azure directory tenant are distinct identities. Never reuse the old globally shared Azure checkpoint file.
- Connector tests must isolate `TENANT_STORE_DIR`, `POLLING_STATE_DIR`, `CONNECTORS_CHECKPOINT_DIR`, `TENANT_PERSIST_DIR` and the delivery database. `SECRET_BACKEND=memory` alone does not isolate the tenant cursor/config SQLite store. Tests importing `tenant_store` before configuration must patch its `STORE_DIR` as well.
- The current migration, historical limitations, verification evidence and local test-state caveat are documented in `docs/JANUSEC_PHASE2_EXECUTION_VERIFIED_2026_09_14.md`.
