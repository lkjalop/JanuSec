...
now = datetime.utcnow()
start_day = (now - timedelta(days=3)).date()
rows = [
...
for r in rows:
    ...
    print('insert', resp.status_code, resp.json())

r = client.get('/api/v1/metrics/precision/daily', params={'tenant_id':'t1', 'start': start_day.isoformat(), 'end': (start_day + timedelta(days=3)).isoformat()}, headers=API_HEADERS)
