...
docker run --name janusec --rm -p 8080:8080 \
...
SSE:      http://localhost:8080/api/v1/stream/decisions
...
Open in browser: http://localhost:8080/api/v1/stream/decisions
...
```bash
curl -s http://localhost:8080/api/v1/decisions/timeline/demo-observe | jq .
```
...
```bash
curl -s http://localhost:8080/metrics | grep -E 'events_ingested_total|pipeline_stage_latency_ms|escalations_total'
```
Key Story Metrics:
...
```bash
curl -s -X POST http://localhost:8080/api/v1/events/eclipse-xdr \
  -H 'Content-Type: application/json' \
