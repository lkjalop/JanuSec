...
def build_narrator_prompt(
    ...
    if kill_chain:
        sections.append(f"## Case chronology (typed relationships, Cycle {cycle})")
        for p in kill_chain:
            ts_str = p.timestamp.strftime("%Y-%m-%d %H:%M UTC") if getattr(p, "timestamp", None) else "unsequenced"
            ...
            candidate_mitre = ", ".join(getattr(p, "candidate_mitre_techniques", [])[:8]) or "N/A"
            sections.append(
                f"{ts_str} | {p.phase} | {p.actor} | "
                f"{p.action[:150]} | outcome {getattr(p, 'action_outcome', 'unknown')} | "
                f"rows {rows_str} | observed MITRE {mitre_str} | candidate MITRE {candidate_mitre}"
            )
            if p.enables_phase_id:
                sections.append(f"  → enables next phase ({p.enables_phase_id})")
        sections.append("")
        sections.append(
            "Write one chronological paragraph. Say 'enabled' or 'caused' only where an observed causal link is supplied. "
            "Otherwise use neutral ordering such as 'was followed by' and state that causality is unconfirmed. "
            "Keep denied, failed, suspected, inferred, and unsequenced activity explicit. Cite timestamps, actors, IPs, "
            "and row references in [brackets]. Do not present candidate MITRE mappings as observed techniques. "
