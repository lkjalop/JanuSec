        ...
        with _CRITIC_LOCK:
            try:
                ...
                result = _client.generate(
                    prompt,
                    # BUGFIX (Phase 1.6): LLMClient.generate consumes `persona_prompt`,
                    # not `system` — the previous system= kwarg fell into **kwargs and
                    # was silently discarded for Ollama, so the critic's adversarial
                    # framing never reached the model (it only survived because it is
                    # duplicated inside the user template). Route it through the real
                    # persona-prompt channel.
                    persona_prompt=_SYSTEM_PROMPT,
                    max_tokens=800,
                    model=_critic_model,
