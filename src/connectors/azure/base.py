...
def checkpoint_path(name: str, cfg: AzureConnectorConfig) -> str:
    ...
    # Provider directory names are not sufficient to separate customer cursors.
    scope = json.dumps([cfg.customer_tenant_id or cfg.tenant_id, cfg.tenant_id,
                        cfg.client_id, cfg.subscription_id, name], separators=(',', ':'))
    base = os.path.join(base, hashlib.sha256(scope.encode()).hexdigest())
