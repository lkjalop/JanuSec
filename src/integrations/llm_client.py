...
def _model_size_timeout_floor(model_name: Optional[str], base: float) -> float:
    """Model-size-aware timeout floor (never lowers `base`):
    30b+ -> 300s, 14b+ -> 180s, any other `:Nb` tag -> 120s.

    Single source of truth for the heuristic that was duplicated inline in the
    constructor and inverted in _ollama_generate (Phase 1.5)."""
    try:
        m = re.search(r':(\d+)b', model_name or '')
        if not m:
            return base
        gb = int(m.group(1))
        if gb >= 30:
...
class LLMClient(BaseLLMClient):
    ...
    def __init__(self, overrides: Optional[Dict[str, Any]] = None):
        ...
        # Scale timeout based on model size (unless an explicit env timeout is set).
        if isinstance(self.ollama_model, str) and not (os.getenv('OLLAMA_TIMEOUT_SECONDS') or os.getenv('LLM_TIMEOUT_SECONDS')):
            self.ollama_timeout = _model_size_timeout_floor(self.ollama_model, self.ollama_timeout)
    ...
    def _ollama_generate(
        ...
        *,
        ...
        if prompt.lstrip().startswith('/no_think') or any(m in selected_model for m in ('qwen3', 'deepseek-r1')):
            payload['think'] = False
        # Phase 1.5 — timeout precedence. A caller-supplied timeout (e.g. a Tier-1
        # prefill's flat 45s) must NOT shrink below the model-size-aware floor:
        # a 14b model legitimately needs ~180s, so a flat 45 would kill every call
        # and force a fallback narrative. The heuristic floor wins; a *longer* caller
        # timeout is still respected. An explicit operator env timeout
        # (OLLAMA_TIMEOUT_SECONDS / LLM_TIMEOUT_SECONDS) already disables the floor
        # upstream, so operator intent is preserved.
        floor = float(self.ollama_timeout)
        if not (os.getenv('OLLAMA_TIMEOUT_SECONDS') or os.getenv('LLM_TIMEOUT_SECONDS')):
            floor = _model_size_timeout_floor(selected_model, floor)
        if timeout is not None:
            request_timeout = max(float(timeout), floor)
        else:
    ...
    def _ollama_generate_with_host(self, host: str, model: Optional[str], prompt: str, max_tokens: Optional[int], generate_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        # BUGFIX (Phase 1.4): every reference below used an undefined name `sess`,
        # so this entire override-host path raised NameError on first use, was
        # swallowed by the broad except, and always ended in ollama_override_failed
        # — i.e. per-request Ollama host override was dead code. Bind the real
        # session (creating one if the constructor could not).
        sess = self._ollama_session
        if sess is None:
            sess = self._ollama_session = requests.Session()
        host = (host or '').rstrip('/')
        candidates = []
        if generate_path:
        ...
        if prompt.lstrip().startswith('/no_think') or any(tok in _m for tok in ('qwen3', 'deepseek-r1')):
            payload['think'] = False
        # BUGFIX (Phase 1.4): this was capped at 10s (min(10.0, ...)), which
        # guaranteed heavy models (e.g. a 14b that legitimately needs 60–180s) timed
        # out on every attempt. Respect the configured Ollama timeout instead, with a
        # small floor so a mis-set 0/1 doesn't make every attempt fail instantly.
        try:
            base_timeout = float(self.ollama_timeout or 60)
        except Exception:
            base_timeout = 60.0
        attempt_timeout = max(15.0, base_timeout)
        last_exc = None
