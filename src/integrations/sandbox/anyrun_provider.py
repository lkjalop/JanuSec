...
class AnyRunProvider(SandboxProvider):
    """AnyRun provider implementation (generic HTTP wrapper).

    Uses `/analyze/file` or `/analyze/url` and `/reports/{id}` endpoints where available.
    """

    def __init__(self, timeout: int = 30, max_retries: int = 3):
        super().__init__(timeout=timeout, max_retries=max_retries)
        name = os.getenv('SANDBOX_NAME', 'anyrun')
    ...
    async def submit(self, file_bytes: bytes | None, filename: str | None, url: str | None) -> str:
        ...
        if file_bytes is not None and filename is not None:
            endpoint = f"{self.base}/analyze/file"
            ...
            endpoint = f"{self.base}/analyze/url"
        ...
        payload = resp.json()
        task_id = payload.get("task_id") or payload.get("id") or payload.get("uuid") or payload.get("report_id")
        if not task_id:
    ...
    async def result(self, task_id: str, poll_interval: float = 2.0, timeout: int = 300) -> dict[str, Any] | None:
        ...
        endpoint = f"{self.base}/reports/{task_id}"
        start = time.time()
        ...
        while True:
            try:
                ...
                LOG.exception("AnyRunProvider polling error for %s: %s", task_id, exc)
