...
STORE_DIR = os.getenv("TENANT_STORE_DIR") or os.path.join(os.path.dirname(__file__), "..", "data", "tenant_store")
...
def _ensure_key() -> bytes:
    if os.path.exists(os.path.join(STORE_DIR, ".rotation-in-progress")):
        raise SecretBackendError("Tenant store key rotation requires recovery or completion")
    kp = _key_path()
    if os.path.exists(kp):
        with open(kp, "rb") as fh:
            key = fh.read()
        # Publicly exposed historical key: never accept it again.
        if hashlib.sha256(key.strip()).hexdigest() == "8f6b935e92065658df12b33f1f86dd39ebfaf94e6090881640d94404a56edd4c":
            raise SecretBackendError("Compromised tenant-store key: rotate offline before use")
        return key
    k = Fernet.generate_key()
    with open(kp, "xb") as fh:
        os.chmod(kp, 0o600)
        fh.write(k)
    return k
