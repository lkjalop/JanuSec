...
def _build_persona_payload(persona_key: str,
    ...
    if persona_key == 'soc_analyst':
        ...
        payload['headline'] = (
            f"{register.get('candidate_control_count', 0)} candidate controls across {n_frameworks} frameworks; "
            "control effectiveness requires review"
        )
        ...
        payload['headline'] = (
            f"{register.get('candidate_control_count', 0)} control concerns require review. "
        ...
        payload['context_blocks'] = [
            {'label': 'Framework citations requiring review', 'value': [fw for fw in cf if fw != 'unmapped_techniques']},
            {'label': 'Critical candidate concerns', 'value': [
...
def _build_audit_actions(narrative: dict, register: dict) -> list[dict]:
    ...
    if p1_controls:
        actions.append({
            ...
            'description': f"Review {len(p1_controls)} priority control concerns against design and operating evidence",
            'tier': 3,
            'tool_command': 'Prepare a candidate finding with evidence references for authorized review',
            'cab_required': False,
            'auto_executable': False,
        })
...
def _build_audit_opinion(narrative: dict, register: dict) -> str:
    """Telemetry cannot issue an audit opinion on control effectiveness."""
    return (
        "Audit opinion not assessed. Technique citations and incident verdicts "
        "require authorized review against applicable control-design and operating evidence."
    )
