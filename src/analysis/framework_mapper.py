...
def build_control_failure_register(narrative: dict,
    ...
    # Technique citations are candidates, not control tests or legal facts.
    ...
    if evidence_rows:
        ...
        for r in evidence_rows:
            ridx = r.get('row_index') if r.get('row_index') is not None else r.get('row_number')
            if ridx is None:
                continue
            row_techs = r.get('mitre_techniques') or r.get('mitre_technique') or r.get('technique_id') or []
    ...
    for control in all_controls:
        control.setdefault('evidence_refs', [])
        control['assertion_status'] = 'candidate'
        control['tag_grade'] = 'observed' if control['evidence_refs'] else 'inferred'
        control['failure_verified'] = False
    # Verified obligations are evaluated by grc.obligation_engine using signed
    # applicability and data-classification snapshots, never narrative guesses.
    ...
    return {
        ...
        'candidate_control_count': len(all_controls),
        'obligation_status': 'insufficient_information',
        'critical_control_count': 0,
        'critical_candidate_count': critical,
