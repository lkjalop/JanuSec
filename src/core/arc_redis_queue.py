"""Minimal Redis-first, filesystem-fallback queue for resign jobs.

Public API:
  - enqueue_job(job: dict) -> str
  - pop_job(timeout: int = 5) -> Optional[tuple(dict, str)]
  - ack_job(payload_str: str) -> None
...
def get_redis_client():
    """Return a Redis client when configured and reachable, else None."""
    if not _REDIS_URL:
        return None
    try:
        import redis
...
def enqueue_job(job: dict) -> str:
    """Enqueue ``job`` returning a stable ``job_id``.

    Tries Redis first and falls back to writing two files into the
    filesystem directory: ``{job_id}.json`` and ``{job_id}.status.json``.
    """
    ...
    if rc is not None:
        try:
            ...
            rc.hset('resign:status', jid, json.dumps({'state': 'queued', 'created_ts': int(time.time()), 'processed': 0}))
            return jid
        except Exception:
            pass
    ...
    with open(os.path.join(d, f"{jid}.status.json"), 'w', encoding='utf-8') as fh:
        fh.write(json.dumps({'state': 'queued', 'created_ts': int(time.time()), 'processed': 0}))
    return jid


def pop_job(timeout: int = 5) -> Optional[Tuple[dict, str]]:
    """Pop a job and return ``(job_dict, raw_payload)`` or ``None``."""
    rc = get_redis_client()
    if rc is not None:
        try:
...
def ack_job(payload_str: str):
    """Acknowledge and remove a filesystem ``.inprog`` job matching payload."""
    rc = get_redis_client()
    if rc is not None:
        return
