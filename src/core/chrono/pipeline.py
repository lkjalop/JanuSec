...
class ChronoAccum:
    ...
    # Supporting evidence for long-horizon byte aggregates.  ChronoGraph used to
    # retain only the counter value, which meant a cumulative exfil finding could
    # influence a verdict but could not become an auditable timeline milestone.
    destination_row_refs: dict[tuple[str, str], set[int]] = field(default_factory=dict)
        ...
        if u:
            ...
            if b > 0:
                ...
                if dst and not dst.startswith(("10.", "172.", "192.168.", "127.")):
                    ...
                    try:
                        acc.destination_row_refs.setdefault((u, dst), set()).add(int(r.get("row_index")))
                    except (TypeError, ValueError):
                        # A counter remains useful without a row reference, but it is
                        # deliberately ineligible for a derived evidentiary phase.
                        pass
            if _is_recon_cmd(r):
                ...
                if dst_sums:
                    ...
                    if top_bytes >= exfil_dst_floor or mimics:
                        ...
                        if row_refs:
                            ...
                            phases = cl.setdefault("phases", [])
                            if not any(
                                isinstance(phase, dict)
                                and phase.get("phase_id") == "cloud_object_exfiltration"
                                for phase in phases
                            ):
                                phases.append(
                                    {
                                        "phase_id": "cloud_object_exfiltration",
                                        "name": "Cumulative Cloud Object Exfiltration",
                                        "case_role": "data_exfiltration",
                                        "severity": "critical",
                                        "row_refs": row_refs,
                                        "row_count": len(row_refs),
                                        "derivation": "chronograph_destination_byte_accumulation",
                                        "direction": "outbound",
                                        "destination": top_dst,
                                        "cumulative_bytes": int(top_bytes),
                                    }
                                )
                            present = cl.setdefault("present_phase_ids", [])
                            if "cloud_object_exfiltration" not in present:
                                present.append("cloud_object_exfiltration")
            except Exception:
                pass
