...
class ChronoSketchStore:
    ...
    def multi_resolution_summary(
        ...
        *,
        reference_ts: Optional[float] = None,
        windows_seconds: tuple[int, ...] = (3600, 86400, 604800, 2592000),
    ) -> Dict[str, Any]:
        """Return an auditable time pyramid for episodic/campaign retrieval.

        This is deliberately descriptive. It does not create causal edges or
        upgrade verdicts, and every window is anchored to the same reference.
        """
        ref = reference_ts if reference_ts is not None else time.time()
        key = (entity_type, entity_id, metric)
        with self._lock:
            emb = self._store.get(key)
        snapshot = emb.snapshot() if emb else {}
        windows: list[dict[str, Any]] = []
        for seconds in sorted({max(BUCKET_SECONDS, int(value)) for value in windows_seconds}):
            start = ref - seconds
            included = {bucket: value for bucket, value in snapshot.items() if start <= bucket <= ref}
            windows.append({
                "window_seconds": seconds,
                "sum": round(sum(included.values()), 6),
                "active_buckets": len(included),
                "first_bucket": min(included) if included else None,
                "last_bucket": max(included) if included else None,
            })
        return {
            ...
            "windows": windows,
            "source": "chronograph_time_pyramid",
        }
