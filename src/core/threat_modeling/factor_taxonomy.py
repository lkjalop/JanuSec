...
_FACTOR_MAP: Dict[str, Dict[str, Any]] = {
    ...
    # Assessment-worker / cluster-narrator factors used by persisted breach reports.
    'cloud:sharepoint_lookalike': {
        'stride': ['spoofing','information_disclosure'],
        'dread': {'damage':0.75,'exploitability':0.7,'discoverability':0.65},
        'maestro': ['initial_access','exfiltration'],
        'mitre': ['T1566.002','T1567.002'],
        'pasta_stage': [3,6],
        'cvss': {'av':'N'},
        'controls': ['NIST:IA-2','NIST:SC-7','NIST:SI-4'],
    },
    'network:sharepoint_subdomain_mismatch': {
        'stride': ['spoofing','information_disclosure','exfiltration'],
        'dread': {'damage':0.75,'exploitability':0.65,'discoverability':0.65},
        'maestro': ['initial_access','exfiltration'],
        'mitre': ['T1566.002','T1567.002'],
        'pasta_stage': [3,6],
        'cvss': {'av':'N'},
        'controls': ['NIST:SC-7','NIST:AU-12','NIST:SI-4'],
    },
    'iam:oauth_consent_grant_suspicious_app': {
        'stride': ['spoofing','elevation','information_disclosure'],
        'dread': {'damage':0.8,'exploitability':0.75,'discoverability':0.6},
        'maestro': ['initial_access','persistence','credential_access'],
        'mitre': ['T1528','T1550.001','T1098.001'],
        'pasta_stage': [3,4],
        'cvss': {'av':'N','pr':'L'},
        'controls': ['NIST:AC-3','NIST:AC-6','NIST:IA-8'],
    },
    'iam:oauth_consent_excessive_scope': {
        'stride': ['spoofing','elevation','information_disclosure'],
        'dread': {'damage':0.78,'exploitability':0.72,'discoverability':0.62},
        'maestro': ['initial_access','credential_access'],
    ...
    'iam:service_principal_credential_add': {
        'stride': ['elevation','persistence','spoofing'],
        'dread': {'damage':0.85,'exploitability':0.7,'discoverability':0.55},
        'maestro': ['persistence','privilege_escalation','credential_access'],
        'mitre': ['T1098.001','T1550.001'],
        'pasta_stage': [4,5],
        'cvss': {'av':'N','pr':'L'},
        'controls': ['NIST:AC-3','NIST:AC-6','NIST:IA-8'],
    },
    'iam:sp_credential_add': {
        'stride': ['elevation','persistence','spoofing'],
        'dread': {'damage':0.85,'exploitability':0.7,'discoverability':0.55},
        'maestro': ['persistence','privilege_escalation','credential_access'],
        'mitre': ['T1098.001','T1550.001'],
    ...
    'iam:as_rep_roasting': {
        'stride': ['elevation','information_disclosure'],
        'dread': {'damage':0.8,'exploitability':0.7,'discoverability':0.65},
        'maestro': ['credential_access'],
        'mitre': ['T1558.004'],
        'pasta_stage': [4,5],
        'cvss': {'av':'N','pr':'L'},
        'controls': ['NIST:AC-3','NIST:IA-5','NIST:SI-4'],
    },
    'iam:kerberoasting': {
        'stride': ['elevation','information_disclosure'],
        'dread': {'damage':0.8,'exploitability':0.75,'discoverability':0.65},
        'maestro': ['credential_access'],
        'mitre': ['T1558.003'],
        'pasta_stage': [4,5],
        'cvss': {'av':'N','pr':'L'},
        'controls': ['NIST:AC-3','NIST:IA-5','NIST:SI-4','NIST:AU-12'],
    },
    'iam:golden_ticket': {
        'stride': ['spoofing','elevation','persistence','lateral_movement'],
        'dread': {'damage':0.95,'exploitability':0.8,'discoverability':0.55},
        'maestro': ['credential_access','privilege_escalation','persistence','lateral_movement'],
        'mitre': ['T1558.001','T1078'],
        'pasta_stage': [4,5],
        'cvss': {'av':'N','pr':'L'},
        'controls': ['NIST:AC-3','NIST:IA-2','NIST:SI-7','NIST:AU-9'],
    },
    'endpoint:wmi_lateral_exec': {
        'stride': ['lateral_movement','tampering','elevation'],
        'dread': {'damage':0.8,'exploitability':0.7,'discoverability':0.6},
        'maestro': ['lateral_movement','execution'],
        'mitre': ['T1047','T1021.003'],
        'pasta_stage': [5],
        'cvss': {'av':'L','pr':'L'},
        'controls': ['NIST:AC-6','NIST:AU-12','NIST:SI-4'],
    },
    'wmi_lateral_movement': {
        'stride': ['lateral_movement','tampering','elevation'],
        'dread': {'damage':0.75,'exploitability':0.68,'discoverability':0.6},
        'maestro': ['lateral_movement','execution'],
        'mitre': ['T1047','T1021.003'],
        'pasta_stage': [5],
        'cvss': {'av':'L','pr':'L'},
    ...
    'recon:sustained_offhours_sequence': {
        'stride': ['spoofing','discovery','elevation'],
        'dread': {'damage':0.65,'exploitability':0.6,'discoverability':0.7},
        'maestro': ['discovery','persistence'],
        'mitre': ['T1078','T1087','T1069'],
        'pasta_stage': [4],
        'cvss': {'av':'N'},
        'controls': ['NIST:AU-6','NIST:SI-4','NIST:AC-2'],
    },
    'discovery:ad_enumeration': {
        'stride': ['information_disclosure','discovery'],
        'dread': {'damage':0.65,'exploitability':0.55,'discoverability':0.75},
        'maestro': ['discovery'],
        'mitre': ['T1087','T1069','T1018'],
        'pasta_stage': [4],
        'cvss': {'av':'L'},
        'controls': ['NIST:AU-12','NIST:SI-4'],
    },
    'exfil:cumulative_bytes_anomaly': {
        'stride': ['information_disclosure','exfiltration'],
        'dread': {'damage':0.9,'exploitability':0.65,'discoverability':0.55},
        'maestro': ['collection','exfiltration'],
        'mitre': ['T1030','T1041','T1567'],
        'pasta_stage': [6,7],
        'cvss': {'av':'N'},
        'controls': ['NIST:SC-7','NIST:AC-4','NIST:AU-12'],
    },
    ...
    'identity:ml_risk_spike': {
        'stride': ['spoofing','elevation'],
        'dread': {'damage':0.6,'exploitability':0.55,'discoverability':0.6},
        'maestro': ['credential_access','privilege_escalation'],
        'mitre': ['T1078'],
        'pasta_stage': [4],
        'cvss': {'av':'N'},
        'controls': ['NIST:SI-4','NIST:AC-2'],
    },
...
def _expand_controls(ctrls: List[str]) -> List[str]:
    # Preserve exact citations; control families do not imply cross-framework
    # equivalence, versions, or customer applicability.
