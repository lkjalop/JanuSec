...
_SKIP_USERS = frozenset({"", "-", "n/a", "na", "unknown", "system", "root", "anonymous", "anonymous_principal"})
...
class EntityResolver:
    def __init__(self, asset_inventory: dict | None = None) -> None:
        ...
        self._ip_user: dict[str, str] = {}       # learned IP -> principal when strongly dominant
    ...
    def build_from_rows(self, rows: list[dict]) -> "EntityResolver":
        ...
        # IPs can be NAT/shared, so require substantially stronger dominance
        # than a host-owner relationship. Ambiguous IPs stay candidate matches.
        for ip, ctr in ip_users.items():
            top, n = ctr.most_common(1)[0]
            if n >= max(5, 0.8 * sum(ctr.values())):
                self._ip_user[ip] = top
        return self
    ...
    def resolve_row(self, row: dict) -> dict:
        ...
        ip = _lower(row.get("src_ip"))
        ac = self.asset_class(host) if host else None
        ...
        owner, method = self.resolve_owner(host) if host else (None, "none")
        if not owner and ip in self._ip_user:
            owner, method = self._ip_user[ip], "ip_cooccurrence_strong"
        if owner:
