...
def get_compliance_hits(factors: List[str]) -> Dict[str, List[str]]:
    """Return candidate control citations for explicitly registered factors.
    ...
    for factor in factors:
        ...
        mapping = factor_control_record(factor)['controls']
        if not mapping:
            continue
...
def get_compliance_report(factors: List[str]) -> Dict[str, Any]:
    ...
    report: Dict[str, Any] = {
        'frameworks_violated': [],
        'frameworks_referenced': sorted(hits),
        'assertion_status': 'candidate',
        'unmapped_factors': sorted(f for f in set(factors) if not get_compliance_hits([f])),
