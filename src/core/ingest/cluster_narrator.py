...
def _temporal_rag_block(cluster: dict, tenant: str = "default") -> str:
    """Cross-assessment corroboration from the tenant TemporalRAG corpus (Phase 3.15).

    Path B (the headline narrator) previously had zero RAG. This retrieves prior
    related activity for the cluster's entities/techniques so the narrative can note a
    broader pattern ("this actor was seen in a prior assessment"). It is CONTEXT ONLY:
    the model is told not to name entities from here — the allow-list + scrub keep all
    named entities grounded in the current cluster's evidence. Safe no-op when the
    corpus is empty (first assessment, or standalone/benchmark calls)."""
    if os.getenv("JANUSEC_TEMPORAL_RAG_NARRATE", "1").lower() not in {"1", "true", "yes"}:
        return ""
    try:
        from src.ai.temporal_rag import get_engine
        parts: list[str] = []
        for k in ("shared_users", "shared_accounts", "shared_hosts", "shared_ips", "mitre_techniques"):
            parts.extend(str(v) for v in (cluster.get(k) or [])[:5])
        query = " ".join(p for p in parts if p).strip()
        if not query:
            return ""
        block = get_engine().build_context_block(query, tenant=tenant, assessment_id=None, mode="historical")
        if not block.get("rag_available"):
            return ""
        lines = [
            "RELATED PRIOR ACTIVITY (retrieved from tenant history — CONTEXT ONLY for",
            "assessing whether this is part of a broader pattern and calibrating",
            "confidence. Do NOT name entities from this section and do NOT cite it in",
            "evidence_refs; name only entities from the evidence rows / allow-list):",
            "  " + str(block.get("summary_hint") or ""),
        ]
        for n in (block.get("neighbours") or [])[:5]:
            lines.append(f"    - {n.get('severity')}: {str(n.get('description'))[:80]}")
        return "\n".join(lines)
    except Exception:
        logger.debug("cluster_narrator: temporal-rag block unavailable", exc_info=True)
        return ""


def _translate_evidence_refs(prompt_positions, evidence: list[dict]) -> list[int]:
    """Map the LLM's 1-based prompt-list positions to absolute row indices, so a
    citation resolves to a real event (and is a subset of the provenance). Positions
    out of range or without a row_index are dropped; order and duplicates preserved."""
    out: list[int] = []
    for p in (prompt_positions or []):
        try:
            idx = int(p) - 1
        except (TypeError, ValueError):
            continue
        if 0 <= idx < len(evidence):
            ri = evidence[idx].get("row_index")
            if ri is not None:
                try:
                    out.append(int(float(ri)))
                except (TypeError, ValueError):
                    continue
    return out
...
def _build_prompt(cluster: dict, evidence_rows: list[dict]) -> str:
    ...
    # Entity-constrained generation (Phase 3.11): an explicit allow-list of the
    # entities in the evidence, injected as a hard constraint so the model is told
    # exactly which names it may use. Backstopped by the deterministic output scrub.
    allowlist_block = ""
    if os.getenv("JANUSEC_ENTITY_ALLOWLIST", "1").lower() in {"1", "true", "yes"}:
        try:
            from src.core.ingest.entity_constraints import build_entity_allowlist, format_allowlist_block
            allowlist_block = format_allowlist_block(build_entity_allowlist(evidence_rows))
        except Exception:
            allowlist_block = ""
    ...
    if campaign_block:
        optional_sections += f"\n{campaign_block}\n"
    rag_block = _temporal_rag_block(cluster, str(cluster.get("_tenant") or "default"))
    if rag_block:
        optional_sections += f"\n{rag_block}\n"
...
def _apply_narrative_to_cluster(cluster: dict, narrative: dict, *, upgrade_only: bool) -> None:
    ...
    _conf = max(0.0, min(1.0, selected_confidence))
    # Verdict-consistency floor: never display a breach verdict with a confidence that
    # contradicts it (e.g. VALIDATED_BREACH at 0.55). The deterministic verdict is
    # detector-backed and authoritative; the LLM's low self-rating must not drag the
    # headline confidence below the verdict's own threshold. Recorded for audit.
    try:
        from src.core.verdicts import confidence_floor as _confidence_floor
        _floor = _confidence_floor(selected_verdict)
        if _conf < _floor:
            cluster["_confidence_pre_floor"] = round(_conf, 3)
            cluster["_confidence_floored"] = True
            _conf = _floor
            narrative["confidence"] = _conf  # keep narrative + cluster consistent
    except Exception:
        pass
    cluster["confidence"] = _conf
...
def narrate_cluster(
    ...
    *,
    ...
    # Carry the tenant so _build_prompt -> _temporal_rag_block queries the right corpus.
    cluster["_tenant"] = tenant
    ...
    # ── Citation unification (describe-only trust fix) ────────────────────────
    # The LLM returns evidence_refs as 1-based POSITIONS in the prompt's evidence list,
    # which an analyst can't map to a raw event and which don't match the absolute-index
    # provenance (_llm_evidence_refs). Translate them to absolute row indices so a
    # citation resolves to a real event and is a subset of the provenance. Keep the raw
    # prompt positions for debugging.
    try:
        _prompt_pos = narrative.get("evidence_refs") or []
        _abs_refs = _translate_evidence_refs(_prompt_pos, evidence)
        if _abs_refs:
            narrative["_evidence_refs_prompt_pos"] = list(_prompt_pos)
            narrative["evidence_refs"] = _abs_refs
    except Exception:
        logger.debug("narrator: evidence_refs translation failed for %s", cluster_id, exc_info=True)
    ...
    # ── Evidence-integrity gate + adversarial re-verification loop ─────────────
    # Authoritative, BLOCKING check (unlike the annotate-only grounding above): if the
    # narrative names entities absent from the evidence, or asserts a cluster entity
    # with no supporting row (swallowed evidence), regenerate ONCE with the specific
    # violations fed back to the model, then fall back to a deterministic grounded
    # summary and flag _integrity_degraded — so a factually-incorrect narrative is
    # never published clean. Gated by JANUSEC_EVIDENCE_INTEGRITY (default on).
    if os.getenv("JANUSEC_EVIDENCE_INTEGRITY", "1").lower() in {"1", "true", "yes"}:
        try:
            from src.core.ingest.evidence_integrity import regenerate_until_grounded
            _min_cit = int(os.getenv("JANUSEC_NARRATE_MIN_CITATIONS", "3"))
            # The absolute evidence row refs are recorded on the cluster; surface them
            # on the narrative so the integrity check counts real citations (not just
            # inline [N] markers).
            if not narrative.get("_llm_evidence_refs") and cluster.get("_llm_evidence_refs"):
                narrative["_llm_evidence_refs"] = cluster.get("_llm_evidence_refs")
            _regen_enabled = os.getenv("JANUSEC_EVIDENCE_INTEGRITY_REGEN", "1").lower() in {"1", "true", "yes"}

            def _regenerate(feedback: str, prev: dict) -> dict:
                retry_prompt = (
                    prompt
                    + "\n\nCORRECTION REQUIRED — your previous answer contained grounding "
                    "errors. " + feedback + "\nRegenerate the COMPLETE JSON using ONLY "
                    "entities (users, hosts, IPs, domains) that appear verbatim in the "
                    "evidence rows above. Do not invent names."
                )
                with _NARRATOR_LOCK:
                    _retry = _client.generate(retry_prompt, **generate_kwargs)
                _raw = (_retry or {}).get("text") if isinstance(_retry, dict) else ""
                if _redactor is not None and _raw:
                    _raw = _redactor.restore(_raw)
                if not (_raw or "").strip():
                    return {}
                _new = _parse_llm_output(_raw, cluster_id)
                # Carry forward deterministic structure the correction shouldn't touch.
                for _k in ("verdict", "kill_chain_stages", "kill_chain_stage",
                           "confidence", "_llm_evidence_refs", "_narrator_model"):
                    if not _new.get(_k) and prev.get(_k) is not None:
                        _new[_k] = prev.get(_k)
                return _new

            def _grounded_fallback(prev: dict, _report) -> dict:
                # Last resort: keep the deterministic structure, replace only the prose
                # with a summary composed from GROUNDED cluster facts. Filter shared
                # users to those actually present in the evidence, so the fallback prose
                # can't itself re-introduce an ungrounded name (e.g. a distractor whose
                # supporting rows were deselected by the evidence cap).
                from src.core.ingest.evidence_integrity import _evidence_token_set, _is_grounded
                _tokens = _evidence_token_set(evidence)
                _users = [u for u in (cluster.get("shared_users") or [])
                          if isinstance(u, str) and _is_grounded(u.lower(), _tokens)]
                users = ", ".join(_users[:3]) or "the flagged actor"
                stages = " -> ".join(prev.get("kill_chain_stages") or _killchain_from_phases(cluster) or [])
                prose = (f"Deterministic summary: activity attributed to {users}"
                         + (f" spanning {stages}." if stages else "."))
                fb = dict(prev)
                fb["attack_narrative"] = prose
                fb["_attack_narrative_source"] = "deterministic_grounded_fallback"
                return fb

            narrative, _integrity_report, _integrity_meta = regenerate_until_grounded(
                narrative, evidence, cluster,
                regenerate_fn=(_regenerate if _regen_enabled else None),
                min_citations=_min_cit,
                max_retries=int(os.getenv("JANUSEC_EVIDENCE_INTEGRITY_MAX_RETRIES", "1")),
                fallback_fn=_grounded_fallback,
                # Swallowed-claim detection grounds against the full row set, so an
                # entity merely deselected by the evidence cap isn't flagged as lost.
                full_evidence_rows=all_evidence_rows,
            )
            cluster["_evidence_integrity"] = _integrity_report.as_dict()
            cluster["_evidence_integrity_meta"] = _integrity_meta
            if not _integrity_report.ok:
                logger.info(
                    "narrator: cluster %s integrity degraded after %d regen attempt(s): %s",
                    cluster_id, _integrity_meta.get("attempts", 0), "; ".join(_integrity_report.issues),
                )
        except Exception as _ie:
            logger.debug("narrator: evidence-integrity loop failed for %s: %s", cluster_id, _ie)

    # ── Entity scrub (Phase 3.11 — structural guarantee) ──────────────────────
    # Final deterministic pass on whatever will ship (LLM output, a regenerated
    # narrative, or a fallback): any entity-shaped token not grounded in the evidence
    # is substituted with the closest real entity (typo near-miss) or redacted to a
    # neutral phrase. After this, the published prose CANNOT contain a fabricated
    # host / IP / domain / user — hallucination is impossible in the output, not just
    # detectable. Gated by JANUSEC_ENTITY_SCRUB (default on).
    if os.getenv("JANUSEC_ENTITY_SCRUB", "1").lower() in {"1", "true", "yes"}:
        try:
            from src.core.ingest.entity_constraints import scrub_ungrounded_entities
            _all_redactions: list[dict] = []
            for _field in ("attack_narrative", "ioc_summary", "what_happened", "root_cause"):
                _val = narrative.get(_field)
                if isinstance(_val, str) and _val.strip():
                    _scrubbed, _red = scrub_ungrounded_entities(_val, evidence)
                    if _red:
                        narrative[_field] = _scrubbed
                        _all_redactions.extend(_red)
            if _all_redactions:
                narrative["_entity_scrub"] = _all_redactions
                cluster["_entity_scrub"] = _all_redactions
                logger.info("narrator: scrubbed %d ungrounded entity mention(s) from cluster %s",
                            len(_all_redactions), cluster_id)
        except Exception as _se:
            logger.debug("narrator: entity scrub failed for %s: %s", cluster_id, _se)

    # ── MITRE-ID correction (describe-only trust fix) ─────────────────────────
    # The platform KNOWS the correct MITRE technique IDs deterministically (from the
    # cluster's factor tags via factor_to_mitre); the LLM routinely ships wrong ones
    # (T1547 for OAuth, T1075 for WMI). Surface the deterministic set authoritatively
    # and strip any T-code from the prose that isn't in it. Gated by
    # JANUSEC_MITRE_CORRECTION (default on); no-op when no deterministic set exists.
    if os.getenv("JANUSEC_MITRE_CORRECTION", "1").lower() in {"1", "true", "yes"}:
        try:
            ...
            _tags = cluster.get("factor_tags") or {}
            _factor_keys = list(_tags.keys()) if isinstance(_tags, dict) else list(_tags)
            # Two vocabularies feed the deterministic technique set: FACTOR_TO_MITRE's
            # 'domain:factor' keys AND the phase-detector 'phase_id' names the cluster's
            # factor_tags actually use — consult both.
            _det_mitre = set(get_all_mappings(_factor_keys).get("mitre", []))
            _det_mitre |= phase_mitre_techniques(_factor_keys)
            _det_mitre |= {str(t).upper() for t in (cluster.get("mitre_techniques") or [])
                           if str(t).upper().startswith("T")}
            if _det_mitre:
                # Authoritative technique list for downstream/UI — correct by construction.
                narrative["mitre_techniques"] = sorted(_det_mitre)
                _mitre_removed: list[dict] = []
                for _field in ("ioc_summary", "attack_narrative", "what_happened", "root_cause"):
                    _val = narrative.get(_field)
                    if isinstance(_val, str) and _val.strip():
                        _corrected, _rem = correct_mitre_ids(_val, _det_mitre)
                        if _rem:
                            narrative[_field] = _corrected
                            _mitre_removed.extend(_rem)
                if _mitre_removed:
                    narrative["_mitre_corrected"] = _mitre_removed
                    cluster["_mitre_corrected"] = _mitre_removed
                    logger.info("narrator: removed %d ungrounded MITRE code(s) from cluster %s: %s",
                                len(_mitre_removed), cluster_id, [r["code"] for r in _mitre_removed])
        except Exception as _me:
            logger.debug("narrator: MITRE correction failed for %s: %s", cluster_id, _me)
...
def narrate_top_clusters(
    ...
    *,
    ...
    The stage is time-budgeted: JANUSEC_INGEST_NARRATE_TIMEOUT_S (default 600s).
    Once the budget is exhausted, remaining clusters receive fallback narratives.
    The per-call timeout is JANUSEC_INGEST_LLM_TIMEOUT_S (default 45s), but note
    that the LLM client floors it to the model-size-aware minimum (a 14b model
    gets >=180s), so the effective per-call budget for the T2 model is ~180s.
    With the adversarial critic (JANUSEC_CRITIC_TIMEOUT_S default 90s), worst-case
    per cluster is ~135s; disable critic on the T1 fast path with
    JANUSEC_CRITIC_T1_SKIP=1.
