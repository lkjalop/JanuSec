...
_STORAGE_COMPONENT_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")
...
def _safe_storage_component(value: object, *, field: str, max_length: int) -> str:
    """Validate a tenant/id before it is used as a filesystem component.

    This is a defense-in-depth boundary for jobs recovered from DuckDB or placed
    on the worker queue by non-HTTP producers. Do not sanitize by replacement:
    doing so can collapse distinct tenant identifiers onto the same path.
    """
    component = str(value or "").strip()
    if (
        not component
        or len(component) > max_length
        or not _STORAGE_COMPONENT_RE.fullmatch(component)
        or component.endswith(".")
        or component.split(".", 1)[0].upper() in _WINDOWS_RESERVED_COMPONENTS
    ):
        raise ValueError(f"invalid_{field}_identifier")
    return component


def _tenant_directory_key(tenant_id: str) -> str:
    """Avoid Windows/macOS case-fold aliases for distinct tenant principals."""
...
def _assert_path_within(base: str, candidate: str) -> None:
    """Reject path traversal and symlink/junction escapes from the storage root."""
    base_real = os.path.realpath(base)
    candidate_real = os.path.realpath(candidate)
    try:
        common = os.path.commonpath([base_real, candidate_real])
    except ValueError as exc:
        raise ValueError("assessment_path_outside_storage_root") from exc
    if os.path.normcase(common) != os.path.normcase(base_real):
        raise ValueError("assessment_path_outside_storage_root")


# ── Shared queue — ingest_endpoints posts here, worker consumes ────────────────
# Bounded so a burst of uploads applies backpressure instead of growing the queue
# without limit (the QueueFull branch in enqueue_job was previously dead code because
# the queue was unbounded). maxsize=0 restores unbounded behaviour as an escape hatch.
_INGEST_QUEUE_MAXSIZE = int(os.getenv("JANUSEC_INGEST_QUEUE_MAX", "256"))
_INGEST_QUEUE: asyncio.Queue = asyncio.Queue(maxsize=_INGEST_QUEUE_MAXSIZE)


class IngestQueueFull(Exception):
    """Raised by enqueue_job when the bounded ingest queue is full — the caller
    (upload endpoint) should surface this as HTTP 503 so the client retries."""

    def __init__(self, assessment_id: str):
        super().__init__(f"ingest queue full — job {assessment_id} rejected")
        self.assessment_id = assessment_id


# Wall-clock ceiling for a single assessment. One stuck job (slow LLM/enrichment)
# must not occupy the single-concurrency worker indefinitely and stall every queued
# assessment behind it. 0 disables the timeout.
_INGEST_JOB_TIMEOUT_S = float(os.getenv("JANUSEC_INGEST_JOB_TIMEOUT_S", "1800"))
...
# Breach-worthy-for-narration verdicts — sourced from the canonical taxonomy so this
# is no longer a 5th independent copy. Includes SUSPECTED_BREACH (a suspected breach
# still gets narrated/seeded) and now INCIDENT (previously missing — an oversight; an
# INCIDENT verdict is a breach outcome and should seed narration/actions too).
try:
    from core.verdicts import NARRATABLE_BREACH_VERDICTS as _BREACH_VERDICTS
except Exception:  # pragma: no cover - dual-import fallback
    from src.core.verdicts import NARRATABLE_BREACH_VERDICTS as _BREACH_VERDICTS
...
def _calibrate_cluster_confidence(cluster: dict) -> None:
    ...
    evidence_count = int(cluster.get("row_count") or len(cluster.get("row_refs") or []) or 0)
    triage_score = float(cluster.get("triage_score") or cluster.get("max_triage_score") or 0.0)
...
def _derive_santos_tier2_aliases(cluster: dict, rows: list[dict]) -> None:
    ...
    combined = " ".join(texts)
    explicit_travel = any(
        term in combined for term in ("impossible travel", "geo-velocity", "geovelocity", "newcountry")
    )
...
def _normalize_ingest_row(raw: dict, row_index: int) -> dict:
    ...
    # Source-controlled fixture labels are evidence *about the fixture*, not
    # detector conclusions. Keep the original bytes in immutable storage but
    # exclude these fields from every production scoring/correlation path.
    safe_raw = dict(raw)
    excluded_annotations: list[str] = []
    if os.getenv("ALLOW_UNSIGNED_FIXTURE_LABELS", "0").lower() not in {"1", "true", "yes"}:
        for field in ("_cluster", "_anomaly", "_severity", "_note", "_risk"):
            if field in safe_raw:
                safe_raw.pop(field, None)
                excluded_annotations.append(field)
        notes = str(safe_raw.get("notes") or "")
        if "cluster:" in notes.lower():
            safe_raw.pop("notes", None)
            excluded_annotations.append("notes:cluster")
    try:
    ...
    row.setdefault("_source", safe_raw.get("_source", ""))
    row.setdefault("source_file", safe_raw.get("_source", ""))
    if not row.get("source_type"):
        row["source_type"] = raw.get("source_type") or raw.get("_source_type") or row.get("_source_type") or ""

    row["triage_score"] = _score_async_ingest_row(safe_raw, row)
    if excluded_annotations:
        row["excluded_source_annotations"] = excluded_annotations
...
def _score_async_ingest_row(raw: dict, normalized: dict) -> float:
    ...
    if source_type in _LOW_NOISE_SOURCE_TYPES:
        event_name_lower = str(
            normalized.get("event_name") or raw.get("eventName") or raw.get("event_type") or raw.get("query_type") or ""
        ).lower()
    ...
    if (
        raw.get("alert_signature")
        or raw.get("detect_id")
        or any(t in event_name for t in ("detect", "rtrexecuted", "alert"))
    ):
    ...
    if _raw_src_type == "windows_security":
        _win_eid = str(raw.get("windows_event_id") or raw.get("event_id") or raw.get("EventID") or "").strip()
        # Security-relevant event IDs — exclude 4624/4634 (high-volume logon/logoff)
        _WIN_SECURITY_EIDS = {
            "4768",
            "4769",
            "4771",  # Kerberos TGT/TGS/failure
            "4625",
            "4648",
            "4672",  # logon failure, explicit logon, special privileges
            "4698",
            "4702",  # scheduled task created/modified
            "4776",
            "4778",
            "4779",  # NTLM auth, session reconnect/disconnect
            "4720",
            "4738",
            "4740",  # account create, change, lockout
            "4728",
            "4732",
            "4756",  # group membership changes (domain/local/universal)
        }
        ...
        if _enc in ("0x17", "0x18") and _win_eid == "4769":
            _target_svc = (
                str(
                    ...
                    or raw.get("target_service_name")
                    or raw.get("target_user_name")
            ...
            _requester = (
                str(raw.get("account_name") or raw.get("user") or raw.get("user_canonical") or "").strip().lower()
            )
            _exclude_svcs = {
                s.strip().lower() for s in os.getenv("KERBEROAST_EXCLUDE_SERVICES", "").split(",") if s.strip()
            }
    ...
    try:
        ...
        bytes_moved = float(raw.get("bytes_sent") or raw.get("orig_bytes") or raw.get("bytes") or 0)
    ...
    try:
        ...
        _dst_h = (
            str(
                ...
                or raw.get("domain")
                or raw.get("tls_sni")
                or normalized.get("dst_host")
                or normalized.get("resp_h")
                or normalized.get("domain")
                or normalized.get("tls_sni")
    ...
    _eid_s1 = str(
        normalized.get("windows_event_id")
        or raw.get("windows_event_id")
        or raw.get("WindowsEventId")
        or raw.get("EventID")
        or raw.get("event_id")
        or ""
    ).strip()
    ...
    _topts_s1_raw = (
        str(
            normalized.get("ticket_options")
            or raw.get("ticket_options")
            or raw.get("TicketOptions")
            or raw.get("ticket_flags")
            or "0x0"
        )
        .strip()
        .lower()
    ...
    _tlife_s1_raw = str(
        normalized.get("ticket_lifetime_hours")
        or raw.get("ticket_lifetime_hours")
        or raw.get("TicketLifetimeHours")
        or raw.get("ticket_lifetime")
        or "0"
    ).strip()
    try:
    ...
    _topts_gt_suspicious = bool((_topts_s1 & _GT_PROXIABLE_FLAG) and (_topts_s1 & _GT_FORWARDABLE_FLAG))
...
def _parse_file_to_store(
    ...
    try:
        for raw_row in _pf(path, filename=filename):
            ...
            _obj = str(
                norm.get("ObjectId") or norm.get("object_id") or norm.get("file_path") or norm.get("resource") or ""
            ).lower()
...
def _collect_lane_factor_tags(rows: list[dict]) -> dict[int, list[str]]:
    ...
    for row in rows:
        ...
        ev_name = (
            str(row.get("event_name") or row.get("EventName") or row.get("operation") or row.get("Operation") or "")
            .lower()
            .replace("-", "")
            .replace("_", "")
        )
        if "inboxrule" in ev_name:
            fwd = str(
                row.get("ForwardingSmtpAddress")
                or row.get("forwarding_smtp")
                or row.get("external_recipient_domain")
                or ""
            ).strip()
        ...
        proc = str(
            row.get("process_name") or row.get("TargetProcessName") or row.get("image") or row.get("process") or ""
        ).lower()
        parent = str(
            row.get("parent_process_name") or row.get("ParentProcessName") or row.get("parent_process") or ""
        ).lower()
        ...
        if (("wmic" in proc or "wmic" in cmdline) and "/node:" in cmdline and "process call create" in cmdline) or (
            "wmiprvse.exe" in parent and ("-enc" in cmdline or "encodedcommand" in cmdline)
        ):
        ...
        if user and len(country) == 2 and country.isalpha():
            ts_raw = str(
                row.get("event_time") or row.get("timestamp") or row.get("date_utc") or row.get("eventTime") or "0"
            )
            try:
...
async def run_assessment_pipeline(
    ...
    *,
    ...
    try:
        ...
        from src.core.ingest.pipeline_ledger import safely_capture

        _ledger_context = await safely_capture(org, assessment_id, file_paths)
                ...
                try:
                    ...
                    if file_lane == LANE_EVALUATION_ANSWER_KEY:
                        quarantined_files.append(
                            {"filename": filename, "lane": file_lane, "reason": lane_result.get("reason", "")}
                        )
                    ...
                    for _r2b in filtered_rows:
                        if str(_r2b.get("user_canonical") or _r2b.get("user") or "") == _u2b:
                            ...
                            if _blk_r in _personal:
                                if not _r2b.get("_janusec_anomaly"):
                                    _r2b["_janusec_anomaly"] = "user_ip_drift"
            ...
            for _r2c in filtered_rows:
                for _ip_fld in ("src_ip", "dst_ip", "external_ip", "client_ip", "remote_ip"):
                    ...
                    if (
                        not _ip2c
                        or _ip2c in ("-", "0.0.0.0", "::")
                        or _ip2c.startswith("10.")
                        or _ip2c.startswith("192.168.")
                    ):
                        continue
                    ...
                    if _meta:
                        ...
                        _prefix = "src_" if _ip_fld == "src_ip" else ("dst_" if _ip_fld == "dst_ip" else "")
                        if _meta.get("asn"):
                        ...
                        if _meta.get("country"):
                            ...
                            if is_high_risk_country(_meta["country"]):
                                ...
                                _r2c["triage_score"] = max(
                                    float(_r2c.get("triage_score") or 0.0),
                                    0.7,
                                )
                                _high_risk_count += 1
        ...
        if _ledger_context is not None:
            try:
                from src.core.ingest.pipeline_ledger import record_parse_normalize

                _execution_contract = await record_parse_normalize(
                    org,
                    assessment_id,
                    _ledger_context,
                ...
                if os.getenv("EVIDENCE_LEDGER_REQUIRED", "0").lower() in {"1", "true", "yes"}:
                    raise
                logger.exception("parse/normalize ledger degraded for %s", assessment_id)
        ...
        if os.getenv("JANUSEC_ASYNC_LEGACY_HYDRATE", "0").lower() in {"1", "true", "yes"}:
            ...
            try:
                ...
                logger.info(
                    ...
                    len(isolated_noise),
                    len(pivot_groups),
                    _diag.get("stale_rows", 0),
                    _diag.get("singleton_drop_count", 0),
                )
        ...
        try:
            ...
            for _cl in clusters:
                ...
                try:
                    ...
                    logger.warning(
                        "cluster intelligence enrichment failed for %s: %s",
                        _cl.get("cluster_id"),
                        _ei_err,
                        exc_info=True,
                    )
        ...
        try:
            ...
            def _row_user_5x(row: dict) -> str:
                return (
                    str(
                        row.get("user_canonical")
                        or row.get("user")
                        or row.get("account_name")
                        or row.get("userPrincipalName")
                        or row.get("user_principal_name")
                        or ""
                    )
                    .strip()
                    .lower()
            ...
            try:
                ...
                for _row_iam in _factor_scan_rows:
                    if not isinstance(_row_iam, dict):
                        continue
                    _src_iam = str(
                        _row_iam.get("source_type") or _row_iam.get("_source_type") or _row_iam.get("log_source") or ""
                    ).lower()
            ...
            try:
                ...
                for _row_cloud in _factor_scan_rows:
                    if not isinstance(_row_cloud, dict):
                        continue
                    _src_cloud = str(
                        _row_cloud.get("source_type")
                        or _row_cloud.get("_source_type")
                        or _row_cloud.get("log_source")
                        or ""
                    ).lower()
                    _event_cloud = str(
                        _row_cloud.get("event_type")
                        or _row_cloud.get("operation")
                        or _row_cloud.get("event_name")
                        or _row_cloud.get("activityDisplayName")
                        or ""
                    ).lower()
            ...
            for _cl in clusters:
                ...
                _srcs_fx = {
                    str(s).strip().lower() for s in ((_cl.get("sources") or []) + (_cl.get("source_types") or [])) if s
                }
                if _global_high_signal_factors_5x and ("iam" in _srcs_fx or "identity_kerberos" in _srcs_fx):
            ...
            if _retro_pairs:
                ...
                logger.info("Stage 5x: severity retrofix elevated %d rows for %s", len(_retro_pairs), assessment_id)
        except Exception as exc:
        ...
        try:
            ...
            for _cl in clusters:
                ...
                _candidate_rows.sort(key=lambda r: float(r.get("triage_score") or 0), reverse=True)
                ...
                _preview: list[dict] = []
                _source_cap = max(
                    4, _EP_CAP // max(1, len({r.get("_source") or r.get("source_file", "") for r in _candidate_rows}))
                )
                ...
                if _preview:
                    ...
                    _ep_count += 1
            logger.info(
                "Stage 5y: evidence_preview populated for %d clusters (%d rows lookup) for %s",
                _ep_count,
                len(_row_lookup_ep),
                assessment_id,
            )
        ...
        try:
            ...
            for _row_bl in filtered_rows:
                ...
                _h = (
                    str(
                        _row_bl.get("host")
                        or _row_bl.get("hostname")
                        or _row_bl.get("src_host")
                        or _row_bl.get("source_host")
                        or ""
                    )
                    .strip()
                    .lower()
            ...
            for _row_bl in filtered_rows:
                ...
                if _u:
                    ...
                    _bytes = float(
                        _row_bl.get("bytes_out")
                        or _row_bl.get("bytes_sent")
                        or _row_bl.get("orig_bytes")
                        or _row_bl.get("bytes")
                        or 0
                    )
                    if _bytes > 0:
        ...
        try:
            ...
            try:
                ...
                logger.warning("post-Chrono threat-case refresh failed for %s: %s", assessment_id, _presentation_exc)
        ...
        try:
            ...
            def _safe_z(chrono, etype, entity, metric, window=86400 * 7):
                try:
                    return float(
                        chrono.z_score(
                            etype, entity, metric, window_seconds=window, reference_ts=(_chrono_ref_ts or None)
                        ).get("z")
                        or 0.0
                    )
                except Exception:
            ...
            if len(_iso_boot_vecs) >= 5:
                try:
                    ...
                    logger.info(
                        "Stage 5g+: ISO bootstrap trained on %d user vectors for %s", len(_iso_boot_vecs), assessment_id
                    )
            ...
            for _cl_gp in clusters:
                ...
                _princ_gp = [
                    str(u).strip().lower()
                    for u in (
                        _cl_gp.get("affected_principals")
                        or _cl_gp.get("shared_accounts")
                        or _cl_gp.get("shared_users")
                        or []
                    )
                    if u
                ][:4]
                ...
                if _triage_boost > 0:
                    _cl_gp["triage_score"] = min(1.0, float(_cl_gp.get("triage_score") or 0.5) + _triage_boost)
        ...
        try:
            ...
            for _cl_5l in clusters:
                ...
                try:
                    ...
                    _princ_5l = list(
                        dict.fromkeys((_cl_5l.get("shared_accounts") or []) + (_cl_5l.get("shared_users") or []))
                    )[:5]
                    _best_ml: dict = {}
                    ...
                    logger.debug("Pattern Synthesizer failed for cluster %s: %s", _cl_5l.get("cluster_id"), _syn_err)
            logger.info(
        ...
        # Stage 5m: freeze case boundaries before any model, persona, executive,
        # or GRC narration sees the evidence. Low-and-slow output remains a
        # candidate signal and cannot upgrade a verdict.
        try:
            ...
            assessment["case_partitions"] = build_case_partitions(
                ...
                analysis_clusters=list(assessment.get("analysis_clusters") or clusters),
                # Long-horizon phases can be supported by individually benign rows
                # outside the 25k clustering cap.  Freeze boundaries against the
            ...
            logger.info(
                ...
                len(assessment["case_chronologies"]),
                len(assessment["campaign_accumulations"]),
                assessment_id,
            )
        except Exception as exc:
            assessment["case_partitions"] = []
            assessment["case_chronologies"] = []
            assessment["campaign_accumulations"] = []
            logger.warning("case partition/chronology construction failed for %s: %s", assessment_id, exc)
        ...
        try:
            ...
            if not _opctx.is_empty:
                ...
                if _suppressed or _elevated:
                    logger.info(
                        "operator context: %d pentest cluster(s) suppressed, "
        ...
        try:
            ...
            def _index_temporal_rag_rows(rows_snapshot: list[dict]) -> int:
                ...
                if not (_rag and rows_snapshot):
                    return 0
                # Per-assessment corpus (existing — used by Path C / exec summary).
                n = _rag.index_rows(rows_snapshot, tenant=org, assessment_id=assessment_id)
                # Tenant-history corpus (assessment_id=None) so a FUTURE assessment's
                # Path B narration can retrieve cross-assessment corroboration for the
                # same tenant (Phase 3.15). Keyed by org, so tenants stay isolated.
                try:
                    _rag.index_rows(rows_snapshot, tenant=org, assessment_id=None)
                except Exception:
                    pass
                return n
        ...
        try:
            ...
            if _hg is not None and hasattr(_hg, "ingest_event"):
                ...
                _edge_count = (
                    _hg.edge_count()
                    if hasattr(_hg, "edge_count")
                    else sum(len(v) for v in (getattr(_hg, "adj", {}) or {}).values())
                )
        ...
        try:
            ...
            for _tc in _threat_cases:
                ...
                if _tc_verdict in _BREACH_VERDICTS:
                    ...
                    if not any(
                        c.get("cluster_id") == _tc.get("cluster_id") or c.get("case_id") == _tc.get("case_id")
                        for c in _seed_clusters
                    ):
                        _seed_clusters.append(_proxy)
            _seed_proposed_actions_and_kill_chain(assessment, _seed_clusters, filtered_rows)
            logger.info(
                "Stage 6b: seeded proposed_actions=%d, kill_chain=%d for %s",
        ...
        # ── Stage 6b2: GRC audit pack — breach findings -> decomposed DREAD +
        # driven actions + auditable Nonconformities (the Control-Assurance view).
        try:
            from src.core.grc.nonconformity import build_audit_pack
            import datetime as _dt_grc

            _lead_grc_case_id = str(assessment.get("kill_chain_case_id") or "")
            _lead_grc_partition = next(
                (
                    item for item in assessment.get("case_partitions") or []
                    if isinstance(item, dict) and str(item.get("case_id") or "") == _lead_grc_case_id
                ),
                None,
            )
            _lead_grc_cluster_ids = {
                str(value) for value in ((_lead_grc_partition or {}).get("supporting_cluster_ids") or [])
            }
            _legacy_grc_clusters = [
                cluster for cluster in _seed_clusters
                if not _lead_grc_cluster_ids
                or str(cluster.get("cluster_id") or cluster.get("case_id") or "") in _lead_grc_cluster_ids
                or str(cluster.get("case_id") or "") == _lead_grc_case_id
            ]
            _legacy_grc_refs: set[int] = set()
            for _legacy_cluster in _legacy_grc_clusters:
                for _legacy_ref in _legacy_cluster.get("row_refs") or []:
                    try:
                        _legacy_grc_refs.add(int(float(_legacy_ref)))
                    except (TypeError, ValueError):
                        continue
            _legacy_grc_rows = []
            for _legacy_row in filtered_rows:
                try:
                    if not _legacy_grc_refs or int(float(_legacy_row.get("row_index"))) in _legacy_grc_refs:
                        _legacy_grc_rows.append(_legacy_row)
                except (AttributeError, TypeError, ValueError):
                    continue
            _audit_pack = build_audit_pack(
                _legacy_grc_clusters,
                rows=_legacy_grc_rows,
                transaction_time=_dt_grc.datetime.now(_dt_grc.timezone.utc).isoformat(),
            )
            if _audit_pack.get("findings"):
                # Step 2: LLM-polish the executive summary (structured-slot narration),
                # validated against the deterministic draft; falls back to it on any drift.
                if _audit_pack.get("executive_summary"):
                    try:
                        from src.core.grc.summary_narrator import narrate_summary

                        _top = max(
                            _audit_pack["findings"],
                            key=lambda f: float((f.get("dread") or {}).get("overall_score") or 0),
                        )
                        _audit_pack["executive_summary"] = narrate_summary(
                            _audit_pack["executive_summary"], finding_mitre=_top.get("mitre")
                        )
                    except Exception as _nexc:
                        logger.debug("GRC summary narration skipped for %s: %s", assessment_id, _nexc)
                assessment["audit_pack"] = _audit_pack
                assessment["nonconformities"] = _audit_pack.get("nonconformities", [])
                logger.info(
                    "Stage 6b2: GRC audit pack — %d finding(s), %d NC(s), %d P1 for %s",
                    _audit_pack["summary"].get("total_findings", 0),
                    _audit_pack["summary"].get("total_ncs", 0),
                    _audit_pack["summary"].get("p1_count", 0),
                    assessment_id,
                )
        except Exception as exc:
            logger.debug("GRC audit-pack seeding skipped for %s: %s", assessment_id, exc)

        # Re-project GRC findings per immutable case boundary. The legacy pack
        # above is retained only when no partition is available.
        try:
            from src.core.grc.nonconformity import build_audit_pack as _build_case_audit_pack
            import datetime as _dt_case_grc

            _audit_packs: dict[str, dict] = {}
            for _partition in assessment.get("case_partitions") or []:
                if not isinstance(_partition, dict) or _partition.get("status") == "background":
                    continue
                _case_id = str(_partition.get("case_id") or "")
                _cluster_ids = {str(value) for value in _partition.get("supporting_cluster_ids") or []}
                _matched_case_clusters = [
                    cluster for cluster in _seed_clusters
                    if str(cluster.get("cluster_id") or cluster.get("case_id") or "") in _cluster_ids
                    or str(cluster.get("case_id") or "") == _case_id
                ]
                _case_clusters = [_cluster_from_case_partition(_partition, _matched_case_clusters)]
                _case_refs: set[int] = set()
                for _case_cluster in _case_clusters:
                    for _ref in _case_cluster.get("row_refs") or []:
                        try:
                            _case_refs.add(int(float(_ref)))
                        except (TypeError, ValueError):
                            continue
                _case_rows = []
                for _row in filtered_rows:
                    try:
                        if isinstance(_row, dict) and int(float(_row.get("row_index"))) in _case_refs:
                            _case_rows.append(_row)
                    except (TypeError, ValueError):
                        continue
                _pack = _build_case_audit_pack(
                    _case_clusters,
                    rows=_case_rows,
                    transaction_time=_dt_case_grc.datetime.now(_dt_case_grc.timezone.utc).isoformat(),
                )
                if not _pack.get("findings"):
                    continue
                _pack["case_id"] = _case_id
                _pack["scope"] = "single_case"
                for _nonconformity in _pack.get("nonconformities") or []:
                    if isinstance(_nonconformity, dict):
                        _nonconformity.setdefault("review_status", "unreviewed")
                        _nonconformity.setdefault("assertion_status", "possible_control_weakness")
                _audit_packs[_case_id] = _pack
            if _audit_packs:
                assessment["audit_packs_by_case"] = _audit_packs
                _lead_case_id = str(assessment.get("kill_chain_case_id") or "")
                _lead_pack = _audit_packs.get(_lead_case_id) or next(iter(_audit_packs.values()))
                assessment["audit_pack"] = _lead_pack
                assessment["nonconformities"] = _lead_pack.get("nonconformities", [])
        except Exception as exc:
            logger.debug("case-scoped GRC audit-pack projection skipped for %s: %s", assessment_id, exc)
        ...
        try:
            _summaries_by_case: dict[str, dict] = {}
            for _partition in assessment.get("case_partitions") or []:
                if not isinstance(_partition, dict) or _partition.get("status") == "background":
                    continue
                _case_id = str(_partition.get("case_id") or "")
                ...
                _case_clusters = [_cluster_from_case_partition(_partition, _matched_case_clusters)]
                _row_refs: set[int] = set()
                for _case_cluster in _case_clusters:
                    for _ref in _case_cluster.get("row_refs") or []:
                        try:
                            _row_refs.add(int(float(_ref)))
                        except (TypeError, ValueError):
                            continue
                _case_rows = []
                for _row in filtered_rows:
                    try:
                        if isinstance(_row, dict) and int(float(_row.get("row_index"))) in _row_refs:
                            _case_rows.append(_row)
                    except (TypeError, ValueError):
                        continue
                _case_summary = _generate_deterministic_executive_summary(
                    assessment, _case_clusters, _case_rows,
                )
                _case_summary["case_id"] = _case_id
                _case_summary["scope"] = "single_case"
                _summaries_by_case[_case_id] = _case_summary
            if _summaries_by_case:
                assessment["executive_summaries_by_case"] = _summaries_by_case
                _lead_summary = _summaries_by_case.get(str(assessment.get("kill_chain_case_id") or "")) or next(iter(_summaries_by_case.values()))
                assessment["executive_summary"] = _lead_summary.get("executive_summary", "")
                assessment["exec_summary_llm"] = _lead_summary
                assessment["compliance_violations"] = _lead_summary.get("compliance_violations") or []
        except Exception as exc:
            logger.debug("case-scoped executive summary projection skipped for %s: %s", assessment_id, exc)

        # Stage 6d: materialize the authoritative, append-only typed graph. The
        # assessment stores its receipt; the graph records remain tenant-scoped
        # in the projection repository.
        try:
            ...
            projection_rows = await asyncio.to_thread(_store.load_all_rows, assessment_id)
            _projection_row_ids = [
                evidence_id_for_row(assessment_id, index, normalize_semantics(dict(row)))
                if isinstance(row, dict) else None
                for index, row in enumerate(projection_rows)
            ]
            _graph_projections: dict[str, dict] = {}
            for _partition in assessment.get("case_partitions") or []:
                if not isinstance(_partition, dict) or _partition.get("status") == "background":
                    continue
                _case_id = str(_partition.get("case_id") or "")
                _allowed_ids = {str(value) for value in _partition.get("evidence_ids") or []}
                _case_projection_rows = [
                    row for row, evidence_id in zip(projection_rows, _projection_row_ids)
                    if evidence_id in _allowed_ids
                ]
                if not _case_projection_rows:
                    continue
                _graph_projections[_case_id] = await persist_assessment_projection(
                    tenant_id=org,
                    case_id=_case_id,
                    rows=_case_projection_rows,
            ...
            assessment["graph_projections_by_case"] = _graph_projections
            _lead_projection_id = str(assessment.get("kill_chain_case_id") or "")
            assessment["graph_projection"] = (
                _graph_projections.get(_lead_projection_id)
                or next(iter(_graph_projections.values()), {"status": "unavailable", "reason": "no_case_projection"})
            )
            logger.info(
                "Stage 6d: %d case-scoped typed graph projection(s); lead %s (%d nodes, %d edges) for %s",
                len(_graph_projections),
                ((assessment["graph_projection"].get("receipt") or {}).get("projection_id")),
                assessment["graph_projection"].get("node_count", 0),
                assessment["graph_projection"].get("edge_count", 0),
                assessment_id,
            )
        except Exception as exc:
            assessment["graph_projection"] = {"status": "unavailable", "error": type(exc).__name__}
            assessment.setdefault("coverage_gaps", []).append(
                "Authoritative typed graph projection could not be persisted."
            )
    ...
    for cl in clusters:
        ...
        if not narrative.get("mitre_techniques"):
            try:
                ...
                if inferred:
                    ...
                    cl["inferred_mitre_techniques"] = inferred
                    narrative["mitre_provenance"] = "inferred"
            except Exception as _mitre_err:
                logger.debug("MITRE inference failed for cluster %s: %s", cl.get("cluster_id"), _mitre_err)
            ...
            for ref in cl.get("row_refs") or []:
                try:
                    ...
                    if k in row_lookup:
                        row = row_lookup[k]
                        if cl_mitre:
                            row["derived_cluster_annotations"] = {
                                "cluster_id": cluster_id_stamp,
                                "mitre_techniques": list(cl_mitre),
                                "provenance": "inferred",
                            }
        ...
        for persona_key, payload in payloads.items():
            try:
                ...
                logger.debug("bitemporal trace failed for %s/%s: %s", cluster_id, persona_key, trace_err)
                cl["persona_dispatch"][persona_key] = payload

        # Step 5: Index for TemporalRAG retrieval
        try:
            sig = _signature_from_narrative(narrative)
            summary = (narrative.get("attack_narrative") or narrative.get("ioc_summary") or "")[:300]
...
def _cluster_from_case_partition(partition: dict, supporting_clusters: list[dict]) -> dict:
    """Project an authoritative case partition into the legacy cluster contract.

    Narrative/report builders still consume cluster-shaped records. Using a
    supporting subcluster directly can invert roles (for example, an AS-REP
    target becomes the principal actor). The partition owns identity, roles,
    phases and evidence scope; supporting clusters only contribute detector
    annotations that do not change that truth.
    """
    ...
    actors = sorted({
        ...
        for item in roles
        if str(item.get("role") or "").lower() == "actor"
        and (item.get("entity") or item.get("entity_id"))
    })
    ...
    for cluster in supporting_clusters:
        factor_tags.update(str(value) for value in cluster.get("factor_tags") or [] if value)
        mitre.update(
            str(value)
            for value in cluster.get("mitre") or cluster.get("mitre_techniques") or []
            if value
        )
        entry_point = entry_point or cluster.get("_entry_point")
        for phase in cluster.get("phases") or []:
            if isinstance(phase, dict) and phase.get("phase_id"):
                phase_records.setdefault(str(phase["phase_id"]), dict(phase))
    for phase_id in partition.get("phase_ids") or []:
        phase_records.setdefault(str(phase_id), {"phase_id": str(phase_id), "row_count": 0})
    verdict = str(partition.get("verdict") or "REQUIRES_INVESTIGATION").upper()
    return {
        "cluster_id": str(partition.get("case_id") or partition.get("partition_id") or "case"),
        "case_id": str(partition.get("case_id") or ""),
        "lead_description": str(partition.get("title") or "Case-scoped breach reconstruction"),
        "name": str(partition.get("title") or "Case-scoped breach reconstruction"),
        "verdict": verdict,
        "final_verdict": verdict,
        "row_refs": list(partition.get("row_refs") or []),
        "row_count": len(partition.get("row_refs") or []),
        "shared_users": actors,
        "shared_accounts": actors,
        "entity_roles": roles,
        "phases": list(phase_records.values()),
        "present_phase_ids": sorted(phase_records),
...
def _seed_proposed_actions_and_kill_chain(
    ...
    cluster_case: dict[str, str] = {}
    for partition in assessment.get("case_partitions") or []:
        if not isinstance(partition, dict) or partition.get("status") == "background":
            continue
        partition_case_id = str(partition.get("case_id") or "")
        for cluster_id in partition.get("supporting_cluster_ids") or []:
            cluster_case[str(cluster_id)] = partition_case_id
    for cl in breach_clusters:
        case_id = cluster_case.get(str(cl.get("cluster_id") or cl.get("case_id") or ""), "unpartitioned")
        cl_rows = []
        for ref in cl.get("row_refs") or []:
            try:
                k = int(float(ref))
        ...
        for tmpl in _ACTION_TEMPLATES:
            action_key = (case_id, tmpl["action_type"])
            if action_key in seen_types:
                continue
            try:
    ...
    # ── Kill chain ────────────────────────────────────────────────────────────
    chronologies = [
        item for item in assessment.get("case_chronologies") or []
        if isinstance(item, dict)
    ]
    if not chronologies:
        from src.core.evidence_contract.chronology import compile_case_chronology

        for cluster_index, cluster in enumerate(breach_clusters):
            cluster_rows: list[dict] = []
            for ref in cluster.get("row_refs") or []:
                try:
                    candidate = row_map.get(int(float(ref)))
                except (TypeError, ValueError):
                    candidate = None
                if candidate is not None:
                    cluster_rows.append(candidate)
            if not cluster_rows:
                continue
            cluster_case_id = str(cluster.get("case_id") or cluster.get("cluster_id") or f"case-{cluster_index + 1}")
            chronologies.append(
                compile_case_chronology(
                    tenant_id=str(assessment.get("tenant_id") or assessment.get("org") or "unknown"),
                    assessment_id=str(assessment.get("assessment_id") or cluster_case_id),
                    case_id=cluster_case_id,
                    rows=cluster_rows,
                )
            )
        assessment["case_chronologies"] = chronologies
    assessment["kill_chain_by_case"] = {
        str(item.get("case_id")): list(item.get("milestones") or []) + list(item.get("unsequenced") or [])
        for item in chronologies
        if item.get("case_id")
    }
    # Legacy consumers receive one explicitly identified lead case only. They no
    # longer receive an assessment-wide chain assembled across case boundaries.
    partition_by_case = {
        str(item.get("case_id") or ""): item
        for item in assessment.get("case_partitions") or []
        if isinstance(item, dict)
    }
    lead = max(
        chronologies,
        key=lambda item: (
            _VERDICT_RANK.get(
                str((partition_by_case.get(str(item.get("case_id") or "")) or {}).get("verdict") or "").upper(),
                0,
            ),
            len({m.get("phase") for m in item.get("milestones") or [] if isinstance(m, dict)}),
            len(item.get("milestones") or []),
            len(item.get("unsequenced") or []),
        ),
        default={},
    )
    assessment["kill_chain_case_id"] = lead.get("case_id")
    assessment["kill_chain"] = list(lead.get("milestones") or []) + list(lead.get("unsequenced") or [])
...
def _generate_deterministic_executive_summary(
    ...
    benign_clusters = [
        cluster for cluster in clusters if _cluster_verdict(cluster) in {"BENIGN_EXPECTED", "INSUFFICIENT_EVIDENCE"}
    ]
    top_cluster = max(
        clusters,
        key=lambda cluster: (
    ...
    total_rows = (
        (assessment.get("evidence_store") or {}).get("row_count") or assessment.get("rows_processed") or len(rows or [])
    )
    source_count = (
    ...
    headline = (
        ...
        if any(
            _cluster_verdict(c) in {"VALIDATED_BREACH", "CONFIRMED_BREACH", "CONFIRMED_INTRUSION"}
    ...
    # Executive narrative is about the lead case, not the whole upload. Using
    # all rows promoted benign users and attack targets into "principal actors".
    _lead_refs = {str(value) for value in (top_cluster.get("row_refs") or [])}
    _narrative_rows = [
        row
        for row in (rows or [])
        if isinstance(row, dict) and (not _lead_refs or str(row.get("row_index")) in _lead_refs)
    ]

    for _r in _narrative_rows:
        ...
        _dom = (
            str(
                _r.get("external_recipient_domain")
                or _r.get("dst_host")
                or _r.get("resp_h")
                or _r.get("destination_host")
                or ""
            )
            .strip()
            .lower()
    ...
    _declared_actors = [
        ...
        for value in (top_cluster.get("shared_users") or top_cluster.get("shared_accounts") or [])
        if str(value).strip()
    ]
    _top_users = _declared_actors[:3] or [u for u, _ in _user_counter.most_common(3)]
    ...
    if _compliance_violations:
        ...
        _compliance_lines.append(
            "Compliance impact: "
            + ", ".join(f"{_n} {_fw} control(s)" for _fw, _n in sorted(_by_fw_summary.items()))
            + "."
        )
    ...
    if _top_domains:
        _narrative_parts.append(f"External destinations: {', '.join(_top_domains)}.")
    if _top_factors:
    ...
    _narrative_parts.append(
        "Verdict distribution: " + (", ".join(f"{k}={v}" for k, v in sorted(verdict_counts.items())) or "none") + "."
    )
...
def _persist_assessment_json(assessment_id: str, org: str, data: dict) -> str | None:
    ...
    try:
        safe_assessment_id = _safe_storage_component(assessment_id, field="assessment", max_length=128)
        safe_org = _safe_storage_component(org or "unknown", field="tenant", max_length=64)
        tenant_directory = _tenant_directory_key(safe_org)
        repo_root = os.getcwd()
...
async def _worker_loop() -> None:
    ...
    while True:
        ...
        try:
            ...
            if _INGEST_JOB_TIMEOUT_S and _INGEST_JOB_TIMEOUT_S > 0:
                try:
                    await asyncio.wait_for(
                        ...
                        timeout=_INGEST_JOB_TIMEOUT_S,
                    )
                except asyncio.TimeoutError:
                    # Don't let a single stuck job wedge the single-concurrency worker.
                    logger.error(
                        "ingest_worker: job %s exceeded %.0fs timeout — marking failed",
                        assessment_id,
                        _INGEST_JOB_TIMEOUT_S,
                    )
                    try:
                        from src.core.ingest import store as _store_to

                        _store_to.update_job(
                            assessment_id,
                            status="failed",
                            error=f"job exceeded {_INGEST_JOB_TIMEOUT_S:g}s timeout",
                        )
                    except Exception:
                        logger.exception("ingest_worker: failed to mark timed-out job %s", assessment_id)
            else:
...
def enqueue_job(
    ...
    *,
    ...
    try:
        ...
        # Now reachable (the queue is bounded). Surface the drop as a failed job so
        # the upload's SSE progress reports it instead of hanging forever.
        _ACTIVE_JOB_IDS.discard(assessment_id)
        logger.error(
            "ingest queue full (maxsize=%d) — job %s dropped",
            _INGEST_QUEUE_MAXSIZE,
            assessment_id,
        )
        try:
            from src.core.ingest import store as _store_qf

            _store_qf.update_job(
                assessment_id,
                status="failed",
                error="ingest queue full — try again shortly",
            )
        except Exception:
            logger.exception("ingest_worker: failed to mark queue-full job %s", assessment_id)
        raise IngestQueueFull(assessment_id)
