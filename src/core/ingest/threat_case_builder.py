...
def build_threat_cases(
    ...
    *,
    ...
    """Preserve evidence; only typed detector output can supply a verdict.

    Free-text labels (including negation, examples and attacker-controlled text)
    do not establish an intrusion. Untyped inventory remains investigable.
    """
    raw_clusters = [dict(c) for c in clusters]
    typed = [c for c in raw_clusters if c.get("cluster_kind")]
    untyped = [c for c in raw_clusters if not c.get("cluster_kind")]
    rows = {}
    for row in evidence_rows:
        if not _is_evidence_row(row):
            continue
        try:
            rows[int(row["row_index"])] = row
        except (KeyError, TypeError, ValueError):
            continue
    if not raw_clusters and rows:
        untyped = [{"cluster_id": "analysis-incomplete", "row_refs": sorted(rows)}]
    incomplete = []
    for i, cluster in enumerate(untyped):
        refs = []
        for ref in cluster.get("row_refs") or cluster.get("rows") or []:
            try:
                idx = int(ref)
            except (ValueError, TypeError):
                continue
            if idx in rows and idx not in refs:
                refs.append(idx)
        cid = cluster.get("cluster_id") or f"analysis-incomplete-{i}"
        incomplete.append({
            **cluster, "cluster_id": cid, "case_id": cid,
            "verdict": "ANALYSIS_INCOMPLETE", "final_verdict": "ANALYSIS_INCOMPLETE",
            "analysis_classification": "analysis_incomplete", "severity": "unknown",
            "confidence": 0.0, "confidence_calibration": "uncalibrated",
            "incident_name": "Unclassified evidence",
            "lead_description": "Evidence requires typed detector analysis",
            "reason_summary": "Descriptions alone cannot establish a breach or authorized activity.",
            "phases": [], "row_refs": refs, "row_count": len(refs),
            "evidence_preview": [rows[r] for r in refs[:evidence_cap]],
        })
    return {
        "raw_correlation_clusters": raw_clusters,
        "analysis_clusters": typed + incomplete,
        "threat_cases": (_build_from_cluster_kind(typed, evidence_rows) if typed else []) + incomplete,
    }


def _campaign_title_short(phases: list[dict], *, confirmed: bool) -> str:
    ...
    if len(seen_roles) >= 3:
        return ("CONFIRMED" if confirmed else "SUSPECTED") + " MULTI-PHASE INTRUSION"
    return " + ".join(seen_roles) if seen_roles else (("CONFIRMED" if confirmed else "SUSPECTED") + " INTRUSION")
...
def _build_from_cluster_kind(
    ...
    for position, row in enumerate(evidence_rows):
        if not isinstance(row, dict):
            continue
        try:
            row_by_idx[int(row.get("row_index", position))] = row
        except (TypeError, ValueError):
            row_by_idx[position] = row

    def observed_source_ips(refs: list[Any]) -> set[str]:
        values: set[str] = set()
        for ref in refs:
            try:
                row = row_by_idx.get(int(ref)) or {}
            except (TypeError, ValueError):
                continue
            for field in ("src_ip", "source_ip", "client_ip", "client_address", "sourceIPAddress"):
                if row.get(field):
                    values.add(str(row[field]).strip().lower())
        return values

    kerberos_target_support: list[dict[str, Any]] = []
    for candidate in clusters:
        phase_ids = {
            str(phase.get("phase_id") or "")
            for phase in (candidate.get("phases") or [])
            if isinstance(phase, dict)
        }
        users = {str(value).lower() for value in (candidate.get("shared_users") or []) if value}
        target_accounts = {
            str(role.get("entity") or "").lower()
            for role in (candidate.get("entity_roles") or [])
            if isinstance(role, dict) and role.get("basis") == "target_account" and role.get("role") == "target"
        }
        if phase_ids == {"kerberoasting"} and users and users <= target_accounts:
            kerberos_target_support.append(candidate)

    kerberos_target_ids = {str(item.get("cluster_id") or "") for item in kerberos_target_support}

    # Attach each AS-REP target observation to at most one campaign. A direct
    # kerberoasting phase is strongest; otherwise require a shared observed
    # source IP and a credential/persistence campaign phase. This preserves the
    # target role without turning the requested service account into an actor.
    support_by_campaign: dict[str, list[dict[str, Any]]] = defaultdict(list)
    compatible_phases = {
        "kerberoasting",
    ...
    campaign_candidates = [
        ...
        if item.get("cluster_kind") == "campaign"
        and str(item.get("cluster_id") or "") not in kerberos_target_ids
    ]
    for support in kerberos_target_support:
        phase_refs = [
            ref
            for phase in support.get("phases") or []
            if isinstance(phase, dict) and phase.get("phase_id") == "kerberoasting"
            for ref in phase.get("row_refs") or []
        ]
        support_ips = observed_source_ips(phase_refs or list(support.get("row_refs") or []))
        ranked: list[tuple[int, int, dict[str, Any]]] = []
        for campaign in campaign_candidates:
            phase_ids = {
                str(phase.get("phase_id") or "")
                for phase in (campaign.get("phases") or [])
                if isinstance(phase, dict)
            }
            campaign_ips = observed_source_ips(list(campaign.get("row_refs") or []))
            shared_ip_count = len(support_ips & campaign_ips)
            direct = "kerberoasting" in phase_ids
            if not direct and not (shared_ip_count and phase_ids & compatible_phases):
                continue
            ranked.append((int(direct), shared_ip_count, campaign))
        if ranked:
            ranked.sort(key=lambda item: (item[0], item[1], len(item[2].get("row_refs") or [])), reverse=True)
            winner = ranked[0][2]
            support_by_campaign[str(winner.get("cluster_id") or "")].append(support)
    ...
    for c in clusters:
        ...
        if str(c.get("cluster_id") or "") in kerberos_target_ids:
            continue

        if kind == "campaign":
            phases = c.get("phases") or []
            supporting_targets = support_by_campaign.get(str(c.get("cluster_id") or ""), [])
            supporting_refs = [ref for item in supporting_targets for ref in (item.get("row_refs") or [])]
            row_refs = list(dict.fromkeys([*row_refs, *supporting_refs]))
            supporting_roles = [
                {**role, "role": "target", "basis": "target_account"}
                for item in supporting_targets
                for role in (item.get("entity_roles") or [])
                if isinstance(role, dict) and role.get("basis") == "target_account"
            ]
            entity_roles = list(c.get("entity_roles") or [])
            for role in supporting_roles:
                if role not in entity_roles:
                    entity_roles.append(role)
            supporting_phases = [
                {**phase, "supporting_cluster_id": str(item.get("cluster_id") or "")}
                for item in supporting_targets
                for phase in (item.get("phases") or [])
                if isinstance(phase, dict)
            ]
            existing_phase_ids = {
                str(phase.get("phase_id") or "") for phase in phases if isinstance(phase, dict)
            }
            for phase in supporting_phases:
                if str(phase.get("phase_id") or "") not in existing_phase_ids:
                    phases.append(phase)
                    existing_phase_ids.add(str(phase.get("phase_id") or ""))
            n_phases = len(phases)
            n_sources = len(c.get("sources") or [])
            source_verdict = str(c.get("final_verdict") or c.get("verdict") or "UNDETERMINED").upper()
            confirmed = source_verdict in {"VALIDATED_BREACH", "CONFIRMED_BREACH", "CONFIRMED_INTRUSION"}
            presentation_verdict = source_verdict if confirmed else (
                "SUSPECTED_BREACH" if source_verdict in {"LIKELY_COMPROMISE", "LIKELY_BREACH", "SUSPECTED_BREACH"}
                else source_verdict
            )
            incident_name = _campaign_title_short(phases, confirmed=confirmed)
            headline = (
            ...
            cases.append({
                ...
                "final_verdict":        presentation_verdict,
                ...
                "phases":               phases,
                "present_phase_ids":    list(dict.fromkeys([
                    *(c.get("present_phase_ids") or []),
                    *[p.get("phase_id") for p in phases if p.get("phase_id")],
                ])),
                ...
                "supporting_cluster_ids": [str(item.get("cluster_id")) for item in supporting_targets],
                "evidence_preview":     [],
                "tier1_prefill": {
                    "incident_name":    incident_name,
                    "headline_subtitle": headline,
