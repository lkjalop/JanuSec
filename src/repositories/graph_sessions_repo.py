...
async def get_graph_session(session_id: str, tenant_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Load a session only through an exact tenant partition.

    Tenantless rows are legacy data and must be migrated deliberately.  Treating
    them as a wildcard turns an incomplete migration into a cross-tenant read.
    """
