...
def enrich_narrative(narrative: dict,
    ...
    # Merge affected_data additively: an earlier stage may have written a block
    # with an empty or partial 'classes' list — augment rather than skip.
    existing_ad = narrative.get('affected_data')
    if not isinstance(existing_ad, dict):
        narrative['affected_data'] = affected_data
    else:
        existing_classes = set(existing_ad.get('classes') or [])
        new_classes = set(affected_data.get('classes') or [])
        merged_classes = sorted(existing_classes | new_classes)
        existing_ad['classes'] = merged_classes
        # Only upgrade sensitivity if it would become stricter
        _SENS_RANK = {'unknown': 0, 'low': 1, 'moderate': 2, 'high': 3, 'crown_jewel': 4, 'critical': 5}
        existing_sens = existing_ad.get('sensitivity') or 'unknown'
        new_sens = affected_data.get('sensitivity') or 'unknown'
        if _SENS_RANK.get(new_sens, 0) > _SENS_RANK.get(existing_sens, 0):
            existing_ad['sensitivity'] = new_sens
        # Backfill missing sub-fields from fresh extraction
        for _k in ('tables', 'record_count_estimate', 'crown_jewel_touched', 'sensitivity_evidence_source'):
            if not existing_ad.get(_k) and affected_data.get(_k):
                existing_ad[_k] = affected_data[_k]
