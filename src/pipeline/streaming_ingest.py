...
_NORMALIZER_VERSION = "1.5"  # 1.5: provider-neutral outbound-email recipient semantics
...
def _normalize_cloud(row: dict) -> dict:
    ...
    r.setdefault('src_ip', _safe(
        row.get('sourceIPAddress') or row.get('callerIpAddress') or row.get('ipAddress') or row.get('source_ip') or
    ...
    if not r.get('timestamp'):
        r['timestamp'] = _safe(row.get('eventTime') or row.get('createdDateTime') or
            row.get('activityDateTime') or row.get('detectedDateTime') or
            row.get('start_time') or row.get('START_TIME') or '')
    status = row.get('status')
    if isinstance(status, dict) and status.get('errorCode') is not None:
        r['outcome'] = 'success' if str(status['errorCode']) == '0' else 'failure'
        r['error_code'] = str(status['errorCode'])
...
_CANON_USER_SKIP = frozenset({
    ...
    'nt authority\\system', 'nt authority/system', 'anonymous', 'anonymous_principal',
})
...
def normalize_row(row: dict, source_type: str | None = None) -> dict:
    ...
    if st == SOURCE_UNKNOWN and row.get("source_type"):
        st = classify_source(str(row["source_type"]))
    ...
    if not r.get('timestamp'):
        r['timestamp'] = _safe(
            ...
            row.get('createdDateTime') or row.get('activityDateTime') or
            row.get('detectedDateTime') or row.get('ts') or ''
        )
    ...
    # Kerberos 4768 with pre-auth disabled names the roast TARGET, not an
    # authenticated requester. Preserve the account as a target and let the
    # client address/host resolve the initiating principal when evidence exists.
    _win_id = str(r.get('windows_event_id') or r.get('EventID') or '')
    _preauth = str(r.get('pre_auth_type') or r.get('PreAuthType') or '')
    if _win_id == '4768' and _preauth in {'0', '0x0'} and r.get('account_name'):
        r['target_account'] = r.get('account_name')
        if not r.get('user'):
            r['user_canonical'] = None
    if not r.get('src_ip') and r.get('client_address'):
        r['src_ip'] = r.get('client_address')
    ...
    if '_ts_epoch' not in r:
        ...
        epoch = event_epoch(r.get('timestamp'))
