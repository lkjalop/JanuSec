...
async def ingest_sensor(sensor: str, request: Request) -> Dict[str, Any]:
    ...
    from src.core.assessment_run import register_entry_safe
    from src.api.tenant_helpers import resolve_tenant_with_default
    tenant_id = resolve_tenant_with_default(request)
    requested_run = request.headers.get('X-Assessment-ID') or request.headers.get('x-assessment-id')
    run = await register_entry_safe(
        tenant_id=tenant_id, case_id=requested_run, entry_kind="connector",
        source=f"sensor:{sensor}", source_native_id=idem_key or f"{sensor}:{time.time_ns()}",
        payload=body_bytes,
    )
    ...
    try:
        ...
        pass
    detail = {'ingested': len(envelopes), 'sensor': sensor, 'pending_batch': len(state['batch']), 'rate': state['rate'].get(sensor, {}), 'assessment_run': run}
