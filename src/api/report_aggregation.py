...
try:
    from core.verdicts import is_flagged, is_autoblock
except Exception:  # pragma: no cover - dual-import fallback
    from src.core.verdicts import is_flagged, is_autoblock  # type: ignore
...
def aggregate_decisions(limit: int = 500, tenant_id: str | None = None) -> dict[str, Any]:
    ...
    if tenant_id:
        items = [d for d in items if (_g(d, 'tenant_id') or os.getenv('DEFAULT_TENANT', 'default')) == tenant_id]
    for d in items:
        ...
        # Flagged / autoblock membership is sourced from the canonical verdict
        # taxonomy (src/core/verdicts.py) so the two aggregation loops in this
        # module — and the ground-truth gate — can never drift apart again.
        if is_flagged(verdict):
            if len(flagged) < 25:
                flagged.append(rec)
        if is_autoblock(verdict):
...
def collect_tabular_sessions(session_ids: list[str], tenant_id: str | None = None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    out = []
    pattern_rollup = {
        'suspicious_cells': 0,
        'base64_matches': 0,
    ...
    for sid in session_ids:
        ...
        if not meta or (tenant_id and meta.get('tenant_id') != tenant_id):
            continue
        ...
        for prefix in ('session-', ''):
            ...
            if matches:
                ...
                for r in rows:
                    ...
                    # BUGFIX (Phase 1.3): this disk-assessment loader used a narrow
                    # verdict set that omitted the breach verdicts
                    # (validated_breach / likely_breach / incident / ...), so a
                    # CONFIRMED BREACH loaded from a persisted assessment was dropped
                    # from the report's flagged list — the same class of bug that once
                    # produced an empty CEO report. Both loops now share the canonical
                    # taxonomy in src/core/verdicts.py.
                    if is_autoblock(verdict):
                        if len(autoblocked) < 25:
                            autoblocked.append(rec)
                    if is_flagged(verdict):
...
def build_ingestion_report(
    ...
    decisions = aggregate_decisions(tenant_id=tenant_id)
    ...
    sessions, pattern_rollup = collect_tabular_sessions(session_ids, tenant_id)
    ...
    sbom_summary = None if tenant_id else _sbom_vuln_rollup(include_suppressed_vex)
    ...
    try:
        if not tenant_id and (os.getenv('REPORT_INCLUDE_GRAPH_SUMMARY','0') or '0').lower() in {'1','true','yes'} and combined_records:
            seed: dict[str, Any] = {}
    ...
    # Legacy global graphs have no tenant ownership contract. Do not append
    # their entities to scoped reports; the case projection supplies owned graphs.
    ...
    try:
        if os.getenv('INCLUDE_RISK_ABLATION'):
            ...
            recent = [d for d in list(_DC.values()) if not tenant_id or
                      ((d.get('tenant_id') if isinstance(d, dict) else getattr(d, 'tenant_id', None)) or 'default') == tenant_id][-5:]
