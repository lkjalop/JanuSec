...
async def ai_test(provider: str = 'ollama') -> dict[str, Any]:
    ...
    statuses = await asyncio.to_thread(
        discover_providers,
        probe_external=requested in {'openai', 'anthropic'},
    )
    status = next((item for item in statuses if item.get('provider') == requested), None)
    if not status:
        raise HTTPException(status_code=404, detail='unsupported_ai_provider')
    return {
        'provider': requested,
        ...
        'external_data_transfer': bool(status.get('external_data_transfer')),
