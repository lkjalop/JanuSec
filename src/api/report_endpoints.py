...
def _build_html_payload(report: dict[str, Any], session_ids: list[str], recipients: list[str]) -> dict[str, Any]:
    ...
    payload = {
        ...
        'rows': report.get('evidence_rows') or report.get('flagged_events', []),
        'summary': {
    ...
    try:
        ...
        click_map = {}
        if fetch_recent_clicks and not meta.get('tenant_id') and not meta.get('assessment_id'):
            try:
...
def _load_persisted_assessment_report_data(assessment_id: str | None, tenant_id: str | None = None) -> tuple[str | None, dict]:
    ...
    Filenames and directory names are discovery hints, never authorization.
    An override is an isolation boundary and must not fall back to production.
    """
    ...
    if assessment_id:
        ...
        if cached is not None:
            return assessment_id, deepcopy(cached) if owned(cached) else {}
        if Path(assessment_id).name != assessment_id or any(c in assessment_id for c in "/\\"):
            return assessment_id, {}
    base = Path(os.getenv("SESSION_PERSIST_DIR") or Path.cwd() / "data" / "assessments")
    loaded = []
    if base.is_dir() and tenant_id:
        for path in base.rglob("*.json"):
            if assessment_id and path.name != assessment_id + ".json":
                continue
            try:
                ...
                if not aid or (assessment_id and aid != assessment_id):
                    continue
                loaded.append((str(data.get("created_at") or aid), aid, data))
            except (OSError, ValueError, TypeError):
                continue
    if loaded:
        _, aid, data = max(loaded, key=lambda item: item[0])
        return aid, data
    return assessment_id, {}
...
async def report_ingestion(
    ...
    auth: Any = Depends(require_api_key),
) -> Any:
    ...
    tenant_id = resolve_tenant_id(request, tenant or request.headers.get('X-Tenant-ID') or request.headers.get('x-tenant-id'), auth=auth)
    state = get_platform_state()

    if session_ids:
        from src.api.upload_endpoints import TABULAR_SESSIONS
        if any((TABULAR_SESSIONS.get(sid) or {}).get('tenant_id') != tenant_id for sid in session_ids):
            raise HTTPException(status_code=404, detail="session_not_found")
    # Explicit snapshots and live decisions are distinct report sources.
    ...
    if assessment_id:
        ...
        report = {
            'generated_at': time.time(), 'tenant_id': tenant_id,
            'source_files': assessment_data.get('source_files') or [],
            'flagged_events': [], 'sample_autoblocked': [], 'alerts': [],
            'verdict_stats': {}, 'severity_distribution': {},
            'top_mitre_techniques': [], 'recommendations': [],
            'meta': {'report_version': 2, 'source': 'assessment_snapshot'},
        }
    else:
        from copy import deepcopy
        report = deepcopy(build_ingestion_report(
            session_ids, include_alerts, limit_alerts, state,
            tenant_id=tenant_id, alerts_offset=alerts_offset,
            include_scenarios=include_scenarios, include_model=include_model,
            include_suppressed_vex=include_suppressed_vex,
            persona=persona_selected, variant=variant,
        ))
    ...
    if assessment_data:
        ...
        if not rows and clusters:
            ...
            for cluster in clusters:
                ...
                if len(rows) >= 500:
                    break
        if rows:
            report['evidence_rows'] = rows[:500]
        # Carry verdict and cluster metadata
        if assessment_data.get('verdict'):
            report['verdict'] = assessment_data['verdict']
        if clusters:
            report['flagged_events'] = [c for c in clusters if c.get('verdict') or c.get('final_verdict')]
        ...
        try:
            ...
            if assessment_view.get('evidence_rows'):
                report['evidence_rows'] = assessment_view['evidence_rows'][:500]
