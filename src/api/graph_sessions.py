...
from .tenant_helpers import resolve_tenant_id, resolve_tenant_with_default
...
def _allow_synthetic_evidence() -> bool:
    """Return whether demo/test-only evidence fabrication is explicitly enabled.

    Synthetic graph evidence is production-disabled even if a compatibility
    switch is accidentally present. Demo environments opt in with both a
    non-production runtime profile and ``SYNTHETIC_EVIDENCE_ENABLED=1``; the
    existing test-helper switch is also an explicit test-only opt-in. Merely
    running under pytest is not a sufficient gate.
    """
    truthy = {'1', 'true', 'yes', 'on'}
    explicit_gate = (
        os.getenv('SYNTHETIC_EVIDENCE_ENABLED', '0').strip().lower() in truthy
        or os.getenv('TEST_HELPERS_ENABLED', '0').strip().lower() in truthy
    )
    explicit_profile = (
    ...
    if explicit_profile:
        non_production = explicit_profile.strip().lower() in {
            'test', 'demo', 'dev', 'development', 'local'
        }
    else:
        # Pytest is only a profile signal.  It never enables synthesis without
        # one of the explicit gates above.
        non_production = 'PYTEST_CURRENT_TEST' in os.environ
    return explicit_gate and non_production


def _request_tenant(request: Request | None, auth: Any = None) -> str:
    """Resolve one trusted tenant for graph-session operations."""

    tenant_id = resolve_tenant_id(request, None, auth=auth)
    if not tenant_id:
        tenant_id = resolve_tenant_with_default(request)
    if not tenant_id:  # defensive: the helpers normally raise before this
        raise HTTPException(status_code=400, detail='tenant_id_required')
    return tenant_id


def _session_for_tenant(session_id: str, tenant_id: str) -> Dict[str, Any] | None:
    """Return a cached session only when its ownership exactly matches."""
...
def _cache_session(session_id: str, data: Dict[str, Any], tenant_id: str) -> Dict[str, Any]:
    """Cache a session after enforcing its persisted tenant boundary."""
...
def _tenant_batch_cache(
    runtime: Any,
    tenant_id: str,
    *,
    allow_legacy_demo_data: bool = False,
) -> Dict[str, Any]:
    from src.api.runtime_state import get_file_batch_analysis

    scoped = get_file_batch_analysis(runtime, tenant_id)
    if not allow_legacy_demo_data:
        return scoped
    legacy = get_file_batch_analysis(runtime)
    if legacy is scoped:
        return scoped
    # Explicit demo/test compatibility only. Tenant-owned entries win.
    return {**legacy, **scoped}


def _tenant_runtime_view(runtime: Any, tenant_id: str, *, allow_legacy_demo_data: bool) -> Any:
    """Expose detector state for one tenant without delegating to globals."""

    if runtime is None or allow_legacy_demo_data:
        return runtime
    from src.api.runtime_state import get_tenant_runtime

    partition = get_tenant_runtime(runtime, tenant_id)
    view = SimpleNamespace(**partition)
    view.nx_threshold_cache = getattr(runtime, 'nx_threshold_cache', 0.35)

    def _owned_events(name: str) -> list[Dict[str, Any]]:
        raw = getattr(runtime, name, ()) or ()
        ...
        for item in raw:
            if not isinstance(item, dict):
                continue
            owner = item.get('tenant_id') or item.get('tenant') or item.get('org')
            if owner == tenant_id:
                owned.append(dict(item))
        return owned

    endpoint_names = ('file_events', 'process_events', 'registry_events', 'service_events', 'network_events')
    for name in endpoint_names + (
        'sanitized_events', 'conn_events', 'auth_events', 'azure_signins',
        'flows', 'cloud_events', 'events', 'dns_queries', 'dns_samples',
        'recent_dns_queries', 'recent_ips', 'ip_observations',
    ):
        setattr(view, name, _owned_events(name))

    recent_files: list[Dict[str, Any]] = []
    for batch in partition.get('file_batch_analysis', {}).values():
        if isinstance(batch, dict) and isinstance(batch.get('files'), list):
            recent_files.extend(item for item in batch['files'] if isinstance(item, dict))
    view.recent_files = recent_files
    view.snapshot_endpoint_events = lambda _tenant_id=None: {
        name: list(getattr(view, name, [])) for name in endpoint_names
    }
    return view
    ...
    if edges:
        ...
        for edge in edges:
            if not isinstance(edge, dict):
                continue
            src = edge.get('source') or edge.get('src') or edge.get('src_id')
            dst = edge.get('target') or edge.get('dst') or edge.get('dst_id')
            if src:
...
def _typed_graph_boundary(
    nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]], *, tenant_id: str
) -> Dict[str, Any]:
    """Translate legacy graph-session presentation objects into typed semantics.

    Unsupported or evidence-free causal candidates are excluded, never renamed.
    The caller may retain the input under ``presentation`` for old browser code.
    """
    from src.core.evidence_contract.correlation import EdgeType, validate_edges

    kind_map = {
        'user': 'principal', 'identity': 'principal', 'principal': 'principal',
        'host': 'asset', 'endpoint': 'asset', 'device': 'asset', 'batch': 'evidence', 'session': 'evidence',
        'ip': 'address', 'network': 'address', 'domain': 'address', 'address': 'address',
        'process': 'process', 'attachment': 'data', 'file': 'data', 'data': 'data',
        'policy': 'authorization', 'group': 'authorization', 'role': 'authorization',
        'event': 'evidence', 'cloud': 'asset', 'resource': 'asset',
    }
    ...
    for raw in nodes:
        ...
        if not node_id or node_id in node_ids:
            continue
        legacy_kind = str(raw.get('kind') or raw.get('type') or raw.get('entity_type') or 'evidence').lower()
        typed_nodes.append({
            'node_id': node_id,
            'tenant_id': tenant_id,
            'kind': kind_map.get(legacy_kind, 'evidence'),
            'labels': [str(raw.get('label') or raw.get('normalized') or node_id)],
            'occurred_at': raw.get('occurred_at') or raw.get('event_ts'),
            'observed_at': raw.get('observed_at') or raw.get('observed_ts'),
            'known_at': raw.get('known_at') or raw.get('ingest_ts'),
            'time_uncertainty_seconds': float(raw.get('time_uncertainty_seconds') or 0),
            'topology': list(raw.get('topology') or []),
    ...
    for raw in edges:
        if not isinstance(raw, dict):
            continue
        source = str(raw.get('source') or raw.get('src') or raw.get('src_id') or '')
        target = str(raw.get('target') or raw.get('dst') or raw.get('dst_id') or '')
        legacy_type = str(raw.get('edge_type') or raw.get('type') or '').lower()
        evidence_ids = [str(value) for value in raw.get('evidence_ids') or []]
        if legacy_type == 'evidence':
            lineage.append({'source': source, 'target': target, 'relation': 'derived_from_session_evidence'})
            continue
        if legacy_type in {'overlap', 'source_ip'}:
            edge_type = EdgeType.CANDIDATE_MATCH.value
            basis = ['canonical_field_overlap'] if legacy_type == 'overlap' else ['source_ip', 'same_event']
        elif legacy_type in {'attach_policy', 'add_to_group', 'authorization', 'exposure'}:
            edge_type = EdgeType.CONFIGURED_EXPOSURE.value
            basis = ['authorization_configuration']
        elif legacy_type in {'spawns', 'invokes', 'email_attachment', 'observed_causal'}:
            if not evidence_ids:
                excluded.append({'candidate': raw, 'reason': 'observed_causal_requires_evidence_ids'})
                continue
            edge_type = EdgeType.OBSERVED_CAUSAL.value
            basis = ['observed_source_semantics']
        else:
            excluded.append({'candidate': raw, 'reason': f'unsupported_legacy_edge_type:{legacy_type or "missing"}'})
            continue
        if source not in node_ids or target not in node_ids:
            excluded.append({'candidate': raw, 'reason': 'typed_edge_endpoint_missing'})
            continue
        candidates.append({
            'source': source, 'target': target, 'edge_type': edge_type,
            'evidence_ids': evidence_ids, 'match_basis': basis,
            'confidence': raw.get('confidence'),
        })
    accepted, validation_exclusions = validate_edges(candidates)
    excluded.extend(validation_exclusions)
    return {
        'schema_version': 'janusec.typed-graph/v1',
        'nodes': typed_nodes,
        'edges': accepted,
    ...
    if isinstance(edges, list):
        for edge in edges:
            ...
            timeline.append({
                ...
                'src': normalized.get('source') or normalized.get('src') or normalized.get('src_id'),
                'dst': normalized.get('target') or normalized.get('dst') or normalized.get('dst_id'),
                'edge_type': normalized.get('edge_type') or normalized.get('type'),
    ...
    try:
        ...
        if record:
            ...
            if isinstance(summary, dict):
                if summary.get('tenant_id') not in (None, tenant_id):
                    return None
...
def _fingerprint_discovery(obj: Dict[str, Any]) -> str:
    import hashlib, json as _json
    try:
        core = {k: obj.get(k) for k in ['tenant_id','session_id','type','signals','confidence']}
        raw = _json.dumps(core, sort_keys=True, separators=(',',':'))
        return hashlib.sha256(raw.encode('utf-8')).hexdigest()[:32]
    except Exception:
            ...
            try:
                if isinstance(f, dict):
                    ...
                    if isinstance(f, str) and f != 'batch_missing':
                        ...
                        continue
                disc['tenant_id'] = summary.get('tenant_id')
...
def _ewma_smooth(
    ...
    *,
    unavailable_batches: Optional[Set[str]] = None,
    now: Optional[float] = None,
    stats: Optional[Dict[str, int]] = None,
) -> Dict[str, Dict[str, float]]:
    """Smooth a correlation matrix against prior values and update history.

    History entries use the runtime-state ``(value, timestamp)`` representation,
    while accepting the legacy scalar/list/dict forms.  Missing batches are not
    observations of zero: their cells stay at the current value and do not
    consume or mutate historical evidence.
    """
    if alpha < 0.0 or alpha > 1.0:
        raise ValueError('invalid_alpha')
    history = history if isinstance(history, dict) else {}
    unavailable = set(unavailable_batches or set())
    ...
    for a, row in matrix.items():
        ...
        for b, val in row.items():
            ...
            if a in unavailable or b in unavailable:
                smoothed[a][b] = current
                continue
            key = f"{a}::{b}"
            previous_entry = history.get(key)
            previous: Optional[float] = None
            try:
                if isinstance(previous_entry, (list, tuple)) and previous_entry:
                    previous = float(previous_entry[0])
                elif isinstance(previous_entry, dict):
                    raw_previous = previous_entry.get('value', previous_entry.get('v'))
            ...
            if previous is None:
                ...
                history_hits += 1
            smoothed[a][b] = value
            history[key] = (value, updated_at)
            history_updates += 1
    if isinstance(stats, dict):
        stats['history_entries_used'] = history_hits
        stats['history_entries_updated'] = history_updates
    return smoothed
...
async def build_session(
    ...
    requested_tenant = payload.get('tenant') if isinstance(payload, dict) else None
    tenant_id = resolve_tenant_id(request, requested_tenant, auth=auth)
    if not tenant_id:
        tenant_id = resolve_tenant_with_default(request)
    try:
        ...
        synthetic_evidence_enabled = _allow_synthetic_evidence()
        analysis_runtime = _tenant_runtime_view(
            runtime,
            tenant_id,
            allow_legacy_demo_data=synthetic_evidence_enabled,
        )
        ...
        for sid in session_ids:
            ...
            if synthetic_evidence_enabled and helper_payload and isinstance(helper_payload, dict):
                ...
                batch_value_sets[sid] = value_sets
                continue
            # Repository fixtures are demo/test evidence and must never become
            # production evidence merely because a caller guessed their ids.
            if path.exists() and synthetic_evidence_enabled:
                try:
        ...
        # Make unavailable input explicit.  Only the opt-in demo/test profile
        # may substitute deterministic entities for a missing batch.
        for sid in session_ids:
        ...
        for sid in session_ids:
            if batch_value_sets[sid] is None:
                if synthetic_evidence_enabled:
                    ...
                    # Deterministic overlap is retained solely for explicitly
                    # gated demo/test batches.
                    ...
                    # Empty sets preserve the requested matrix shape without
                    # inventing an entity, edge, factor, or observed zero.
                    batch_value_sets[sid] = {f: set() for f in _CANONICAL_FIELDS}
        ...
        for sid in session_ids:
            ...
            try:
                ...
                overlap_row[sid] = 0.0
            # Legacy demo/test compatibility: an empty inline payload used to
            # receive invented self-coverage.  Keep that only behind the gate.
            try:
                if synthetic_evidence_enabled and overlap_row[sid] == 0.0 and batch_source.get(sid) == 'inline':
                    overlap_row[sid] = 5.0
            except Exception as _exc:
        ...
        # Synthetic overlap guarantee for explicitly gated demo/test batches.
        try:
            if synthetic_evidence_enabled and session_ids and all(s.startswith('batch-overlap-') for s in session_ids):
                any_positive = any(
                    a != b and v > 0
        ...
        try:
            ...
            if runtime is not None:
                if detect_rare_parent_child:
                    try:
                        rp = detect_rare_parent_child(analysis_runtime)
                        for r in (rp or []):
                            try:
                ...
                if detect_file_encryption_wave:
                    try:
                        fw = detect_file_encryption_wave(analysis_runtime)
                        for f in (fw or []):
                            try:
                ...
                if detect_unsigned_network_launch:
                    try:
                        ul = detect_unsigned_network_launch(analysis_runtime)
                        for u in (ul or []):
                            try:
                ...
                if detect_file_entropy_writes:
                    try:
                        fe = detect_file_entropy_writes(analysis_runtime)
                        for e in (fe or []):
                ...
                if detect_shadowcopy_commands:
                    try:
                        vss_cmds = detect_shadowcopy_commands(analysis_runtime)
                ...
                if detect_vss_service_tamper:
                    try:
                        vss_svcs = detect_vss_service_tamper(analysis_runtime)
                ...
                if detect_file_write_rate_spike:
                    try:
                        bursts = detect_file_write_rate_spike(analysis_runtime)
                ...
                if orchestrate_ransomware_signals:
                    try:
                        fusion = orchestrate_ransomware_signals(analysis_runtime, detected_factors)
                        if fusion:
            ...
            if summarize_ransomware_context and runtime is not None:
                try:
                    ransomware_metrics = summarize_ransomware_context(analysis_runtime, detected_factors)
                except Exception:
        ...
        if use_ewma and adaptive_enabled and (provided_alpha in (None, '')):
            try:
                ...
                ewma_alpha = base_alpha
        ewma_stats: Dict[str, int] = {
            'history_entries_used': 0,
            'history_entries_updated': 0,
        }
        ewma_history: Dict[str, Any] = {}
        ewma_uses_legacy_demo_history = False
        if use_ewma:
            try:
                from src.api.runtime_state import get_ewma_history, get_tenant_runtime
                tenant_runtime = get_tenant_runtime(runtime, tenant_id)
                ewma_history = tenant_runtime.setdefault('ewma_history', {})
                if synthetic_evidence_enabled:
                    # Explicit compatibility profile: keep the historical
                    # global demo curve used by legacy fixtures and tests.
                    ewma_history = get_ewma_history(runtime)
                    ewma_uses_legacy_demo_history = True
            except Exception:
                ewma_history = {}
            if not isinstance(ewma_history, dict):
                ewma_history = {}
            smoothed = _ewma_smooth(
                overlap_matrix,
                ewma_alpha,
                ewma_history,
                unavailable_batches=missing_batches if not synthetic_evidence_enabled else None,
                stats=ewma_stats,
            )
            try:
                if ewma_uses_legacy_demo_history:
                    from src.api.runtime_state import save_ewma_history
                    save_ewma_history(runtime)
                else:
                    from src.api.runtime_state import persist_tenant_runtime
                    persist_tenant_runtime(runtime, tenant_id)
            except Exception:
                pass
        else:
            smoothed = None
        ...
        positive_links = sum(
            ...
            if a != b and float(value or 0.0) > 0.0
        )
        if correlate and avg_diversity > 0.5 and positive_links > 0:
        ...
        for mb in sorted(list(missing_batches)):
            gap = {
                ...
                'batch_id': mb,
                'status': 'unavailable',
                'reason': 'No inline, helper, runtime, or permitted fixture evidence was found for the referenced batch.',
                'synthetic_evidence_generated': synthetic_evidence_enabled,
            }
            coverage_gaps.append(gap)
            # Preserve the legacy factor only in explicitly synthetic demo/test
            # mode.  Production output represents absence as a coverage gap,
            # never as a finding.
            if synthetic_evidence_enabled:
                fobj = {
                    'batch_id': mb,
                    'factor': 'batch_missing',
                    'reason': 'referenced batch not found on server',
                    'score': 0.0,
                }
                factors.append(fobj)
        ...
        try:
            missing_count = sum(1 for sid in session_ids if sid in missing_batches)
            all_missing = (missing_count == len(session_ids)) if session_ids else False
            if (not all_missing) or synthetic_evidence_enabled:
                nx_tracker = getattr(analysis_runtime, 'nx_rate_tracker', None) if analysis_runtime is not None else None
                if analysis_runtime is not None and nx_tracker and (hasattr(nx_tracker, '__len__') and len(nx_tracker) > 0):
                    highest_rate = 0.0
                    producer = None
                    ...
                    try:
                        nx_thr = float(getattr(analysis_runtime, 'nx_threshold_cache', os.getenv('ZEEK_NXDOMAIN_RATE_THRESHOLD','0.35') or 0.35))
                    except Exception:
                        try:
        ...
        if synthetic_evidence_enabled and missing_factor_objs and 'multi_source_correlation' in factors:
            chain_bonus += 0.04
        ...
        if sequencer and synthetic_evidence_enabled:
            try:
        ...
        try:
            ...
            try:
                dns_f = dns_exfil_factors(analysis_runtime)
                for d in (dns_f or []):
                    try:
                ...
                if runtime is not None:
                    ...
                    for bid in session_ids:
                        ...
                        for f in (data.get('files') or []):
                            all_files.append(f)
                hash_f = file_hash_rarity_factors(analysis_runtime, all_files)
                for h in (hash_f or []):
                    try:
            ...
            # Beaconing detector
            try:
                beacons = detect_beaconing(analysis_runtime)
                for b in (beacons or []):
                    try:
            ...
            try:
                nxs = detect_nxdomain_spike(analysis_runtime)
                for n in (nxs or []):
                    try:
                        factors.append(n)
                        try:
            ...
            try:
                if detect_office_macro_powershell:
                    for f in (detect_office_macro_powershell(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                ...
                if detect_amsi_bypass:
                    for f in (detect_amsi_bypass(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                ...
                if detect_encoded_powershell:
                    for f in (detect_encoded_powershell(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                ...
                if detect_scheduled_task_lolbin:
                    for f in (detect_scheduled_task_lolbin(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                ...
                if analyze_email_attachments:
                    for f in (analyze_email_attachments(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                ...
                if detect_privilege_escalation:
                    for f in (detect_privilege_escalation(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                            try:
                                confidence = min(0.99, confidence + float(f.get('score',0))*0.25)
                ...
                if score_cloudtrail_events:
                    for f in (score_cloudtrail_events(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                        except Exception as _exc:
                ...
                if detect_transitive_dependency_vuln:
                    for f in (detect_transitive_dependency_vuln(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                        except Exception as _exc:
                            logger.debug('silent_swallow at %s:%d: %s', __file__, 1963, _exc)
                if detect_behavior_mismatch:
                    for f in (detect_behavior_mismatch(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                        except Exception as _exc:
                            logger.debug('silent_swallow at %s:%d: %s', __file__, 1969, _exc)
                if detect_model_provenance:
                    for f in (detect_model_provenance(analysis_runtime) or []):
                        try:
                            detected_factors.append(f)
                        except Exception as _exc:
            ...
            # ASN rarity detector
            try:
                asns = detect_asn_rarity(analysis_runtime)
                for a in (asns or []):
                    try:
            ...
            try:
                if detect_email_bec:
                    becs = detect_email_bec(analysis_runtime)
                    for b in (becs or []):
                        try:
            ...
            try:
                if detect_iam_abuse:
                    iamfs = detect_iam_abuse(analysis_runtime)
                    for f in (iamfs or []):
                        try:
                            detected_factors.append(f)
                            try:
            ...
            try:
                ...
                if runtime is not None:
                    ...
                    for bid in session_ids:
                        ...
                        for f in (data.get('files') or []):
                            all_files.append(f)
                mismatches = detect_file_signature_mismatch(analysis_runtime, all_files)
            ...
            try:
                ent = detect_entropy_and_exfil(analysis_runtime)
                for e in (ent or []):
                    try:
                        detected_factors.append(e)
                        try:
            ...
            try:
                lmv = detect_lateral_movement(analysis_runtime)
                for l in (lmv or []):
                    try:
            ...
            try:
                pc = detect_privilege_changes(analysis_runtime)
                for p in (pc or []):
                    try:
            ...
            try:
                ppa = detect_port_protocol_anomalies(analysis_runtime)
                for p in (ppa or []):
                    try:
                        detected_factors.append(p)
                    except Exception as _exc:
            ...
            try:
                cma = detect_cloud_metadata_anomalies(analysis_runtime)
                for c in (cma or []):
                    try:
            ...
            try:
                events = list(getattr(analysis_runtime, 'sanitized_events', []) or [])
            except Exception:
                events = []
            try:
            ...
            try:
                tod = detect_time_of_day_anomalies(analysis_runtime)
                for t in (tod or []):
                    try:
        ...
        try:
            ...
            node_list = [
                {'id': sid, 'type': 'batch', 'label': sid}
                for sid in session_ids
                if synthetic_evidence_enabled or sid not in missing_batches
            ]
            edge_list = []
        ...
        insufficient_evidence = bool(
            not synthetic_evidence_enabled
            and session_ids
            and len(missing_batches) == len(session_ids)
        )
        if insufficient_evidence:
            # Absence of all requested evidence is not a benign assessment and
            # schema/mapping configuration must not manufacture confidence.
            confidence = 0.0
        ...
        summary = {
            ...
            'batch_sources': batch_source,
            'coverage_gaps': coverage_gaps,
            'coverage_complete': not bool(coverage_gaps),
            'synthetic_evidence_enabled': synthetic_evidence_enabled,
            'diversity_scores': diversity_scores,
            'correlation': overlap_matrix,
            'correlation_smoothed': smoothed,
            'ewma_alpha': ewma_alpha if use_ewma else None,
            'ewma_history_entries_used': ewma_stats.get('history_entries_used', 0) if use_ewma else 0,
            'ewma_history_entries_updated': ewma_stats.get('history_entries_updated', 0) if use_ewma else 0,
            'ewma_history_scope': (
                'legacy_demo' if ewma_uses_legacy_demo_history else 'tenant'
            ) if use_ewma else None,
            'overlap_details': overlap_details or None,
            'factors': normalized_factors,
            'confidence': round(confidence, 4),
            'verdict': verdict,
            'confidence_band': 'unknown' if insufficient_evidence else _band(confidence),
        ...
        evidence_cov = 0.0 if insufficient_evidence else min(
            1.0,
            max(0.0, (domain_diversity_score + mapping_semantics_score) / 2.0),
        )
        ...
        try:
            ...
            chain_session_ids = session_ids if synthetic_evidence_enabled else [
                sid for sid in session_ids if sid not in missing_batches
            ]
            summary['graph_summary']['chains'] = build_multi_stage_chain(chain_session_ids, overlap_matrix)
        except Exception as _exc:
        ...
        try:
            ...
            for a, rows in (overlap_details or {}).items():
                ...
                try:
                    test_helpers = synthetic_evidence_enabled
                except Exception:
                    test_helpers = False
            try:
        ...
        try:
            ...
            if correlate and not any(adj.values()) and len(node_list) > 1:
                if synthetic_evidence_enabled:
                    # Compatibility for explicit demo/test scenarios only.
                    ...
                    summary['graph_summary']['edge_count'] = len(edge_list)
                elif not any(gap.get('type') == 'no_observed_overlap' for gap in coverage_gaps):
                    coverage_gaps.append({
                        'type': 'no_observed_overlap',
                        'status': 'not_observed',
                        'batch_ids': [node.get('id') for node in node_list],
                        'reason': 'No positive cross-batch overlap was observed; no fallback edge was generated.',
        ...
        try:
            if analysis_runtime is not None:
                counts = getattr(analysis_runtime, 'fp_factor_counts', None)
                if not isinstance(counts, dict):
                    counts = {}
                    try:
                        analysis_runtime.fp_factor_counts = counts
                    except Exception as _exc:
        ...
        # Persist only the server-resolved tenant partition (best-effort).
        try:
            tenant = tenant_id
            if tenant:
                try:
                ...
                if isinstance(td, list) and td:
                    for d in td:
                        try:
                            ...
                            # Scope is injected by the trusted runner; test
                            # payloads cannot choose another tenant.
        ...
        if insufficient_evidence:
            # Later optional enrichers must not turn a wholly unavailable input
            # into a positive or benign assessment.
            summary['confidence'] = 0.0
            summary['graph_confidence'] = 0.0
            summary['verdict'] = 'insufficient_evidence'
            summary['confidence_band'] = 'unknown'

        # Authoritative API boundary: only typed graph semantics leave the
        # backend. Legacy shapes survive solely as an explicit presentation
        # adapter for older static pages.
        legacy_graph_summary = dict(summary.get('graph_summary') or {})
        legacy_graph = graph if isinstance(graph, dict) else {'nodes': [], 'edges': []}
        combined_nodes = list(legacy_graph_summary.get('nodes') or [])
        combined_edges = list(legacy_graph_summary.get('edges') or [])
        if not insufficient_evidence:
            combined_nodes += list(legacy_graph.get('nodes') or [])
            combined_edges += list(legacy_graph.get('edges') or [])
        typed_graph = _typed_graph_boundary(combined_nodes, combined_edges, tenant_id=tenant_id)
        response_graph = typed_graph
        if insufficient_evidence:
            response_graph = _typed_graph_boundary(
                list(legacy_graph.get('nodes') or []),
                list(legacy_graph.get('edges') or []),
                tenant_id=tenant_id,
            )
        summary['presentation'] = {
            'legacy_graph_summary': legacy_graph_summary,
            'legacy_graph': legacy_graph,
            'adapter_version': 'janusec.legacy-graph-presentation/v1',
        }
        summary['graph_summary'] = typed_graph
        graph = response_graph
        try:
            store.save(session_id, summary)
        except Exception as _exc:
            logger.debug('typed graph session persistence failed for %s: %s', session_id, _exc)
        compat = {
            'session_id': session_id,
            'summary': summary,
            'graph': graph,
            ...
            'synthetic_evidence_enabled': summary.get('synthetic_evidence_enabled', False),
        }
...
async def list_sessions(
    ...
    try:
        ...
        ttl = 0
    now = time.time()
    tenant_id = _request_tenant(request, auth)
    items = []
    ...
    return {'sessions': items, 'count': len(items), 'ttl_seconds': ttl, 'tenant_id': tenant_id}
    ...
    if not data:
        ...
        try:
            ...
            if rec and isinstance(rec, dict):
                loaded = rec.get('summary') or rec
                if isinstance(loaded, dict) and loaded.get('tenant_id') == tenant_id:
                    data = _cache_session(session_id, loaded, tenant_id)
        except Exception:
            data = None
        if not data:
...
async def get_session_paths(session_id: str, request: Request = None, auth=Depends(require_api_key)) -> Dict[str, Any]:
    ...
    if not data and record and isinstance(record.get('summary'), dict):
        data = _cache_session(session_id, record['summary'], tenant_id)
    if not data:
        raise HTTPException(status_code=404, detail='not_found')
...
async def replay_session(session_id: str, request: Request = None, auth=Depends(require_api_key)) -> Dict[str, Any]:
    ...
    _cache_session(session_id, summary, tenant_id)
...
async def explain_session(session_id: str, as_of: str | None = None, request: Request = None, auth=Depends(require_api_key)) -> Dict[str, Any]:
    ...
    """
    tenant_id = _request_tenant(request, auth)
    rec = _session_for_tenant(session_id, tenant_id)
    if not rec:
        db_record = await _load_graph_session_record(session_id, tenant_id)
        if db_record and isinstance(db_record.get('summary'), dict):
            rec = _cache_session(session_id, db_record['summary'], tenant_id)
    if not rec:
        try:
            ...
            if loaded:
                candidate = loaded.get('summary') or loaded
                if isinstance(candidate, dict) and candidate.get('tenant_id') == tenant_id:
                    rec = _cache_session(session_id, candidate, tenant_id)
        except Exception:
            rec = None
    if not rec:
        raise HTTPException(status_code=404, detail='not_found')
    ...
    explain_coverage_gaps: List[Dict[str, Any]] = [
        dict(gap) for gap in (rec.get('coverage_gaps') or []) if isinstance(gap, dict)
    ]
    try:
        ...
        hotspots = []
    # Hotspot fabrication is available only in the explicitly gated demo/test
    # profile. Production explanations expose the evidence gap instead.
    try:
        sids = rec.get('session_ids') or list((corr or {}).keys()) or []
        if not hotspots and len(sids) >= 2 and _allow_synthetic_evidence():
            if all(str(s).startswith('batch-overlap-') for s in sids):
                fcount = 0
                try:
                    fcount = int((corr.get(sids[0], {}) or {}).get(sids[1], 0) or 0)
                except Exception:
                    fcount = 0
                hotspots = [{'a': sids[0], 'b': sids[1], 'fields': fcount if fcount > 0 else 3}]
            else:
                hotspots = [{'a': sids[0], 'b': sids[1], 'fields': 1}]
        elif not hotspots and len(sids) >= 2:
            if not any(gap.get('type') == 'no_observed_overlap' for gap in explain_coverage_gaps):
                explain_coverage_gaps.append({
                    'type': 'no_observed_overlap',
                    'status': 'not_observed',
                    'batch_ids': list(sids),
                    'reason': 'No positive cross-batch overlap supports a hotspot.',
                    'synthetic_evidence_generated': False,
                })
    except Exception as _exc:
    ...
    try:
        ...
        elif len(rec.get('session_ids') or []) >= 2:
            parts.append('No supported cross-batch overlap was observed; no fallback hotspot was generated.')
        missing_ids = [
            str(gap.get('batch_id'))
            for gap in explain_coverage_gaps
            if gap.get('type') == 'missing_batch' and gap.get('batch_id') is not None
        ]
        if missing_ids:
            parts.append(f"Coverage gap: unavailable batches {', '.join(missing_ids)}.")
        if volatility > 0.5:
    ...
    out = {
        ...
        'coverage_gaps': explain_coverage_gaps,
        'synthetic_evidence_enabled': bool(rec.get('synthetic_evidence_enabled', False)),
        'narrative': narrative,
...
async def session_narrative(
    ...
    data = await explain_session(session_id, request=request, auth=auth)
...
async def list_discoveries(
    ...
    tenant_id = _request_tenant(request, auth)
    if not _session_for_tenant(session_id, tenant_id):
        record = await _load_graph_session_record(session_id, tenant_id)
        if not record:
            raise HTTPException(status_code=404, detail='not_found')
    items = [
        d for d in _DISCOVERIES.values()
        if d.get('session_id') == session_id and d.get('tenant_id') == tenant_id
    ]
    return {
        'session_id': session_id,
        'tenant_id': tenant_id,
...
async def get_discovery(
    ...
    if not d or d.get('tenant_id') != tenant_id:
        raise HTTPException(status_code=404, detail='not_found')
    return d
...
async def cleanup_sessions(
    ...
    tenant_id = _request_tenant(request, auth)
    if session_id:
        if _session_for_tenant(session_id, tenant_id):
    ...
    return {'removed': removed, 'remaining': remaining, 'tenant_id': tenant_id}
