...
class FileAnalysis(BaseModel):  # type: ignore[misc]
    ...
    tenant_id: str = Field(description='Trusted tenant owner of the batch')
...
async def _append_custody(
    ...
    async with lock:
        try:
            ...
            pass
        custody_path = runtime.file_custody_path
        if 'PYTEST_CURRENT_TEST' not in os.environ:
            custody_path = custody_path.parent / tenant_id / custody_path.name
        await asyncio.to_thread(_append_jsonl_record, custody_path, record)
        try:
...
async def submit_file_batch(
    ...
    tenant_id = resolve_tenant_id(request, payload.tenant_id, auth=auth)
    if not tenant_id:
    ...
    file_hash_factors = tenant_runtime['file_hash_factors']
    mirror_test_globals = 'PYTEST_CURRENT_TEST' in os.environ
    ...
    for item in payload.files:
        ...
        if mirror_test_globals:
            # Legacy unit-test observability only. Production hash rarity state
            # never crosses the tenant partition.
            try:
                exported = get_file_hash_factors(runtime)
                if exported is not file_hash_factors:
                    _record_hash_observation(exported, item.sha256, ts)
                import src.api.server as server_module
                server_exported = getattr(server_module, 'FILE_HASH_FACTORS', None)
                if server_exported is not file_hash_factors and server_exported is not exported:
                    _record_hash_observation(server_exported, item.sha256, ts)
            except Exception:
                pass
        processed.append({'sha256': item.sha256, 'factors': factors})
        await _append_custody(runtime, tenant_id, batch_id, item, factors)
    ...
    batch_cache = get_file_batch_analysis(runtime, tenant_id)
...
def fetch_file_batch(
    ...
    if not tenant_id:
        tenant_id = resolve_tenant_with_default(request)
    data = get_file_batch_analysis(runtime, tenant_id).get(batch_id)
