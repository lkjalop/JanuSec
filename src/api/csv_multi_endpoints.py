...
async def upload_csv(file: UploadFile = File(...), request: Request = None, mapping: Optional[str] = Form(None)):
    ...
    tenant_id = resolve_tenant_with_default(request) if request is not None else "default"
    run = await register_entry_safe(
        tenant_id=tenant_id, case_id=None, entry_kind="csv", source="csv_multi",
        source_native_id=file.filename or "upload.csv", payload=content,
    )
