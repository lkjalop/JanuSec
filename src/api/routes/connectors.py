...
def _append_events(runtime, tenant: str, connector_id: str, events: list[dict[str, Any]]) -> None:
    if connector_id in {'entra_signin', 'entra_audit'}:
        from src.repositories.connector_receipts_repo import deliver
        deliver(tenant, 'azure:' + connector_id, events)
...
def _runtime_events_to_assessment_rows(
    ...
    *,
    ...
    from src.repositories.connector_receipts_repo import recent, fingerprint
    buckets.append(recent(tenant))
    ...
    for bucket in buckets:
        for event in reversed(bucket):
            ...
            if connector_filter and connector not in connector_filter:
                continue
            identity = (connector_id, fingerprint(event))
...
def _build_live_lane_assessment(
    *,
    ...
    # The canonical case workspace requires the same owned ingest job and rows
    # as upload assessments. A cache-only report is not a usable live case.
    from src.core.ingest import store as ingest_store
    from src.core.ingest.assessment_worker import _persist_assessment_json
    ingest_store.create_job(assessment_id, tenant)
    ingest_store.persist_row_batch(assessment_id, rows)
    assessment['assessment_id'] = assessment_id
    assessment['org'] = assessment['tenant_id'] = tenant
    if not _persist_assessment_json(assessment_id, tenant, assessment):
        ingest_store.update_job(assessment_id, status='error', error='live_assessment_persistence_failed')
        raise HTTPException(503, 'live_assessment_persistence_failed')
    ingest_store.update_job(assessment_id, status='ready', stage='complete', percent=100, row_count=len(rows))
    return assessment
...
def _dedupe_events(runtime, tenant: str, provider: str, connector: str, events: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
    if provider == 'azure' and connector in {'entra_signin', 'entra_audit'}:
        from src.repositories.connector_receipts_repo import known, fingerprint
        seen = known(tenant, provider + ':' + connector)
        filtered = []
        for event in events:
...
def connector_poll(
    ...
    try:
        ...
        persist_tenant_runtime(runtime, tenant)
        if isinstance(conn, EntraIDConnector):
