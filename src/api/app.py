...
try:
    from .case_intelligence_endpoints import router as case_intelligence_router
    app.include_router(case_intelligence_router)
except Exception as _exc:
    logger.debug('case_intelligence_router_unavailable: %s', _exc)
try:
    from .infrastructure_truth_endpoints import router as infrastructure_truth_router
    app.include_router(infrastructure_truth_router)
except Exception as _exc:
    logger.debug('infrastructure_truth_router_unavailable: %s', _exc)
try:
    from .grc_bridge_endpoints import router as grc_bridge_router
    app.include_router(grc_bridge_router)
except Exception as _exc:
    logger.debug('grc_bridge_router_unavailable: %s', _exc)
try:
...
def register_core_routers(full: bool = True):
    ...
    try:
        from .grc_endpoints import router as grc_router
        app.include_router(grc_router)
        logger.info('Included grc_router into app (lite)')
    except Exception:
        logger.debug('grc_router include failed (lite)')
    try:
...
DEFAULT_FRONTEND = os.getenv('DEFAULT_FRONTEND', 'console').lower()
...
async def serve_root():
    if DEFAULT_FRONTEND in {"console", "live", "workspace", "breach", "investigate"}:
        resp = _serve_static_frontend('janusec-platform-complete-LIVE.html', 'Case Workspace')
        if resp is not None:
            return resp

    # Explicit React mode remains available while the workspace migration is validated.
