...
def get_tenant_runtime(runtime: ServerRuntime | None = None, tenant_id: str | None = None) -> dict:
    ...
    if not tenant_id or str(tenant_id).lower() in {'global','none',''}:
        return {
            ...
            'fp_factor_counts': runtime.fp_factor_counts,
            'fp_factor_fp_labels_counts': runtime.fp_factor_fp_labels_counts,
            'last_access': time.time()
        }
    tid = str(tenant_id).strip()
    if (
        not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]{0,63}', tid)
        or tid in {'.', '..'}
        or tid.endswith('.')
        or tid.split('.', 1)[0].upper() in _WINDOWS_RESERVED_COMPONENTS
    ):
        ...
        try:
            ...
            if tenant_file.exists():
                with tenant_file.open('r', encoding='utf8') as fh:
                    data = json.load(fh)
                for key in ('ewma_history', 'fp_labels', 'beacon_scores'):
                    if isinstance(data.get(key), dict):
                        tmap[key].update(data[key])
        except Exception:
            pass
    else:
        tmap.setdefault('file_batch_analysis', {})
        tmap.setdefault('fp_factor_counts', {})
        tmap.setdefault('fp_factor_fp_labels_counts', {})
...
def get_file_batch_analysis(
    runtime: ServerRuntime | None = None,
    tenant_id: str | None = None,
) -> dict[str, dict[str, Any]]:
    if tenant_id and str(tenant_id).strip().lower() not in {'global', 'none'}:
        tenant_runtime = get_tenant_runtime(runtime, tenant_id)
        return tenant_runtime.setdefault('file_batch_analysis', {})
