...
async def start_replay_diff(payload: Dict[str, Any], request: Request, background: BackgroundTasks = None) -> Dict[str, Any]:
    ...
    run = await register_json_entry_safe(
        tenant_id=str(tenant or "default"), case_id=sid, entry_kind="replay",
        source="replay_diff", source_native_id=sid, document=payload,
    )
    meta['assessment_run'] = run
    ...
    if not disable_bg:
        try:
            ...
            try:
                ...
                pass
    return {'session_id': sid, 'status': 'queued', 'tenant': tenant, 'assessment_run': run}
