...
# _flatten_row_payload → src.api.deep_analyze.enrichment (imported above)
# _TS_FIELDS, _DESC_FIELDS, _ACCOUNT_FIELDS, _HOST_FIELDS, _IP_FIELDS, _RESOURCE_FIELDS,
# _EMAIL_RE, _IP_RE, _MITRE_RE, _SEV_RANK, _LOW_VALUE_ACCOUNT_PIVOTS,
# _ACCOUNT_NESTED_PATHS, _HOST_NESTED_PATHS, _IP_NESTED_PATHS,
# _safe_identity_text, _is_low_value_account_pivot → src.api.deep_analyze.enrichment (imported above)


# _nested_get → src.api.deep_analyze.helpers (imported above)
# _extract_first_value, _parse_backend_timestamp, _is_private_ip_text,
# _extract_from_typed_array, _account_fallback_text, _extract_accounts_backend,
# _extract_hosts_backend, _extract_ips_backend, _extract_resources_backend,
# _extract_tags_backend, _classify_backend_severity, _severity_label_for_rows,
# _infer_cloud_provider, _infer_plane, _extract_cloud_context,
# _extract_identity_context, _extract_network_context, _extract_policy_change_context,
# _extract_guest_onboarding_context, _extract_security_posture_context
# → src.api.deep_analyze.enrichment (imported above)


# _derive_human_validation_required, _VERDICT_HVR_MAP, _apply_hvr_gating,
# _initial_cluster_verdict → src.api.deep_analyze.verdict_seed (imported above)
# _build_remediation_simulation → src.api.deep_analyze.cluster_builder (imported above)


# _extract_event_context, _build_bitemporal_trace, _extract_evidence_mode, _extract_freshness
# → src.api.deep_analyze.enrichment (imported above)


# _cluster_controls_and_frameworks → src.api.deep_analyze.cluster_builder (imported above)
# _normalize_assessment_rows → src.api.deep_analyze.enrichment (imported above)


# _VENDOR_EGRESS_PREFIXES, _is_vendor_egress_ip, _is_ioc_confirmed, _extract_attacker_ips
# → src.api.deep_analyze.enrichment (imported above)


# _build_pair_reason → src.api.deep_analyze.cluster_builder (imported above)


# _build_cluster_lead_description → src.api.deep_analyze.cluster_builder (imported above)
# _build_cluster_reason_summary → src.api.deep_analyze.cluster_builder (imported above)

# _CLUSTER_BUCKET_CAP → src.api.deep_analyze.cluster_builder (imported above)
...
# _build_enrichment_guided_cases → src.api.deep_analyze.cluster_builder (imported above)
# _build_inverted_index → src.api.deep_analyze.cluster_builder (imported above)
# _build_correlation_clusters → src.api.deep_analyze.cluster_builder (imported above)
# _build_task_entry → src.api.deep_analyze.cluster_builder (imported above)
...
def _hydrate_assessment_semantics(assessment: dict) -> dict:
    ...
    # Default ON; set DISABLE_INCIDENT_SPLIT=1 to revert to legacy behaviour.
    if os.getenv('DISABLE_INCIDENT_SPLIT', '0').lower() not in ('1', 'true', 'yes'):
        try:
            from src.core.incident_extraction import extract_incidents
            incidents = extract_incidents(clusters, normalized_rows)
            if incidents:
                # Attach as additive metadata; do not replace clusters yet
                # (UI/verdict-engine still consume correlation_clusters).
                assessment['incidents'] = [
                    inc.to_dict() if hasattr(inc, 'to_dict') else inc.__dict__
                    for inc in incidents
                ]
                logger.info(
                    'incident_extraction: %d clusters → %d incidents',
                    len(clusters), len(incidents),
                )
        except Exception as exc:
            logger.warning('incident_extraction skipped (non-fatal): %s', exc)
